...
if dimension>1:
    lin_params=np.zeros(len_initial);lin_params[0]=1
...
filename="../outputs/nokineticdata_dimension=%d_dt=%.3f_epsilon=%.3f_lambda=%.3f_initlen=%d_startpos=%.2f.h5"%(dimension,timestep,epsilon,lambda_,len_initial,start_pos)
err_t0=0
energy_t0=0
norm_t0=1
...
timeEvolver=TimeEvolution(wave_function,h=timestep,T=Tfinal,epsilon=epsilon,t0=t0,error_t0=err_t0,
                          filename=filename,dimension=dimension,norm=norm_t0,energy=energy_t0)
timeEvolver.time_evolver(type="nokinetic")
