...
fidelity_file_nokinetic = (
    "../../outputs/nokineticfidelity_dimension=%d_dt=%.3f_epsilon=%.3f_lambda=%.3f_len1=%d_len2=%d_startpos=%.2f_noKinetic.h5.npz"
    % (dimension, timestep, epsilon, lambda_, reference, i, startpos)
)
fidelity_file_kinetic = (
    "../../outputs/fidelity_dimension=%d_dt=%.3f_epsilon=%.3f_lambda=%.3f_len1=%d_len2=%d_startpos=%.2f.h5.npz"
    % (dimension, timestep, epsilon, lambda_, reference, i, startpos)
)
fidelity = np.load(fidelity_file_nokinetic)["autocorrelation"]
autocorrelation_file = np.load(
    "../../outputs/autocorrelation_dimension=2_dt=0.010_epsilon=1000.000_lambda=0.112_initlen=20_startpos=2.00_nokinetic.h5.npz"
)
...
titles = [
    r"Autocorrelation $|\langle \Psi^*(t/2)|\Psi(t/2)\rangle|$",
    r"Spectrum $S(\omega)$",
    "Fidelity $F^2_{20,40}(t)$",
]
