...
if not parallel == "parallel" and not parallel == "noKinetic":
    ...
    filename = "../../outputs/data_dimension=%d_dt=%.3f_epsilon=%.3f_lambda=%.3f_initlen=%d_startpos=%.2f.h5" % (
        dimension,
        timestep,
        epsilon,
        lambda_,
...
if len_initial == 1:
    plt.title(
        r"N=%d, %d Gaussian, $\lambda=%.6f$, dt=%f, pos=%.1f" % (dimension, len_initial, lambda_, timestep, startpos)
    )
else:
    plt.title(
        r"N=%d, %d Gaussians, $\lambda=%.6f$, dt=%f, pos=%.1f" % (dimension, len_initial, lambda_, timestep, startpos)
    )
plt.xlabel("Energy")
...
if parallel == "noKinetic":
    ...
    plt.savefig(
        "HHG_dim=%d_Lambda=%.3f_nd=%dstartpos=%d_dt=%.3f.pdf" % (dimension, lambda_, len_initial, startpos, timestep)
    )
    plt.savefig(
        "HHG_dim=%d_Lambda=%.3f_nd=%dstartpos=%d_dt=%.3f.png" % (dimension, lambda_, len_initial, startpos, timestep)
    )
