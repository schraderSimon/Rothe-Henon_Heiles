...
if not parallel == "parallel" and not parallel == "noKinetic":
    ...
    filename = (
        ...
        % (dimension, timestep, epsilon, lambda_, len_initial, startpos)
    )
...
if dimension == 2 and lambda_ == 0.111803:
    simen_reference_2D = np.load(
        "reference_data/hehe_split_step_fourier_acorr_dim_2_xmax_25_ng_512_dt_0.01.npz"
    )
...
if dimension == 3 and lambda_ == 0.111803:
    simen_reference_3D = np.load(
        "reference_data/hehe_split_step_fourier_acorr_dim_3_xmax_25_ng_128_dt_0.01.npz"
    )
    acorr_simen = simen_reference_3D["acorr2"]
if dimension == 4 and lambda_ == 0.111803:
    simen_reference_4D = np.load(
        "reference_data/hehe_split_step_fourier_acorr_dim_4_xmax_25_ng_64_dt_0.01.npz"
    )
...
if not parallel == "noKinetic":
    outname = (
        "../../outputs/autocorrelation_dimension=%d_dt=%.3f_epsilon=%.3f_lambda=%.3f_initlen=%d_startpos=%.2f.h5"
        % (dimension, timestep, epsilon, lambda_, len_initial, startpos)
    )
else:
    outname = (
        "../../outputs/autocorrelation_dimension=%d_dt=%.3f_epsilon=%.3f_lambda=%.3f_initlen=%d_startpos=%.2f_nokinetic.h5"
        % (dimension, timestep, epsilon, lambda_, len_initial, startpos)
    )
...
try:
    ...
    pass
    print("Something went wrong in plotting reference data.")
...
if len_initial == 1:
    plt.title(
        r"N=%d, %d Gaussian, $\lambda=%.6f$, dt=%f, pos=%.1f"
        % (dimension, len_initial, lambda_, timestep, startpos)
    )
else:
    plt.title(
        r"N=%d, %d Gaussians, $\lambda=%.6f$, dt=%f, pos=%.1f"
        % (dimension, len_initial, lambda_, timestep, startpos)
    )
plt.xlabel("Energy")
...
if parallel == "noKinetic":
    plt.savefig(
        "HHG_dim=%d_Lambda=%.3f_nd=%dstartpos=%d_dt=%.3f_noKinetic.pdf"
        % (dimension, lambda_, len_initial, startpos, timestep)
    )
    plt.savefig(
        "HHG_dim=%d_Lambda=%.3f_nd=%dstartpos=%d_dt=%.3f_noKinetic.png"
        % (dimension, lambda_, len_initial, startpos, timestep)
    )
else:
    plt.savefig(
        "HHG_dim=%d_Lambda=%.3f_nd=%dstartpos=%d_dt=%.3f.pdf"
        % (dimension, lambda_, len_initial, startpos, timestep)
    )
    plt.savefig(
        "HHG_dim=%d_Lambda=%.3f_nd=%dstartpos=%d_dt=%.3f.png"
        % (dimension, lambda_, len_initial, startpos, timestep)
    )
