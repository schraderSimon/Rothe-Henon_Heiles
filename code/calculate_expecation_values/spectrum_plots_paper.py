...
for i in range(len(ng)):
    outname = (
        "../../outputs/autocorrelation_dimension=%d_dt=%.3f_epsilon=%.3f_lambda=%.3f_initlen=%d_startpos=%.2f.h5.npz"
...
fig, axes = plt.subplots(
    n_plots, 1, figsize=figsize, constrained_layout=True, sharex=True, sharey=True
)  # Reduced height
...
fig.suptitle(rf"Spectra $S(\omega)$ for D={dimension}")
...
for i in range(n_plots):
    abs_diff = np.abs(np.real(yfs[i][: N // 2]) - np.real(yf_Simen[: NSimen // 2])) / max(
        np.abs(np.real(yf_Simen[: NSimen // 2]))
    )
...
plt.ylabel("Abs. Diff.")
plt.xlabel("Energy")
plt.title(rf"Absolute Difference in Spectra for D={dimension}")
...
plt.xlabel("Energy")
plt.title(rf"Relative Difference in Spectra for D={dimension}")
if lambda_ == 0.111803 and dimension == 2:
    for ax in axes:
        plt.xlim(0, 15)
elif lambda_ == 0.111803 and dimension == 3:
