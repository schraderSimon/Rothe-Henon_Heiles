...
class TimeEvolution():
    def __init__(self,wave_function,h,T,epsilon,t0,error_t0,filename,dimension,max_num_Gaussians=20,norm=1,energy=0):
            ...
            if not os.path.isfile(filename):
                ...
                with h5py.File(filename, "r+") as data_file:
                    ...
                    if t0>(times_read[-1]+1e-5):
                        ...
                        del data_file["rothe_error"]; del data_file["times"]; del data_file["energies"]; del data_file["normalizations"]
    ...
    def make_mask_function(self,nonlin_params):
        ...
        optimization_WF=WF(full_nonlin,full_lin,lambda_=self.lambda_,calculate_Gradient=False,h=0.00,onlyX1X2=True)
    ...
    def minimize_transformed_bonds(self,error_function,gradient,maxiter,start_params,args,gtol=0,multi_bonds=5e-1,printx=False):
        ...
        try:
            solver = minimize(transformed_error, transformed_params, method="BFGS", jac=transformed_gradient,
                                options={"maxiter": maxiter, "gtol": gtol, "hess_inv0": hess_inv0,"c1":1e-4,"c2":0.9},callback=callback_func)
    ...
    def time_evolve(self):
        ...
        if self.t<0.1 or self.removed_at_prev_timestep<1:
            ...
            best_nonlin_params, niter,rotheerror=self.minimize_transformed_bonds(opt_func,
                                                                self.rothe_gradient,
                                                                maxiter=maxiter,
        ...
        if self.rank==0:
            ...
            solver=minimize(optimization_in_c,c_init,jac=deriv_in_c,constraints=[{"type":"eq","fun":constraint1},
        ...
        if sqrt(mask_err)>1e-3:
            ...
            start=time.time()
            solver=minimize(func,best_nonlin_params,method="BFGS",jac=self.rothe_gradient_mask,
        ...
        if self.t>mask_start:
            ...
            left=np.where(abs_blp/np.max(abs_blp)<1e-6)[0] #Those that do not contribute (e.g. 1e-6 of the maximum coefficient)
        else:
            left=[]
    ...
    def time_evolve_noKinetic(self):
        ...
        nonlin_propagated,c_propagated=propagate_kinetic_analytical(self.params_old_nonlin.reshape(-1,self.dimension*(self.dimension+3)),self.params_old_lin,self.h/2)
        ...
        if self.t<0.1 or self.removed_at_prev_timestep<1:
            start=time.time()
            solver = minimize(opt_func, nonlin_params_initial, method="BFGS", jac=opt_grad,
                                options={"maxiter": maxiter, "gtol": gtol, "hess_inv0": hess_inv0})
            best_nonlin_params = solver.x
            niter=solver.nit
            end=time.time()
        ...
        if self.rank==0:
            ...
            solver=minimize(optimization_in_c,c_init,method="SLSQP",jac=deriv_in_c,constraints=[{"type":"eq","fun":constraint1},{"type":"eq","fun":constraint2}],
                            options={"maxiter":200,"ftol":1e-10})
            c_new=solver.x[:len(solver.x)//2]+1j*solver.x[len(solver.x)//2:]
            best_lin_params=c_new
            new_RE=sqrt(optimization_in_c(solver.x))
        ...
        self.comm.bcast(best_lin_params,root=0) #Share with other nodes
        nonlin_propagated,c_propagated=propagate_kinetic_analytical(best_nonlin_params.reshape(-1,n*(n+3)),best_lin_params,self.h/2)
