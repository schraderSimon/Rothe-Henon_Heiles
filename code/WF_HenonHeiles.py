...
import numpy as np
from numpy import conj, einsum, imag, log, pi, real, sqrt
...
def update_overlap_matrix(
    overlap_matrix, pairs_of_indices, matrices, vectors, sum_matrices, sum_matrices_inv
):
    ...
    diagonal_exponents = (
        np.einsum("ak,akl,al->a", bvec_calc, diagonal_sum_matrices_inv, bvec_calc)
        - vmips
        - vmips.conj()
    )
    ...
    exp_constants = (
        vmips_outer[row_indices, col_indices]
        + 0.5 * diagonal_exponents_outer[row_indices, col_indices]
    )
    exponents = (
        np.einsum(
            "ia,iab,ib->i", bvecs, sum_matrices_inv[row_indices, col_indices, :, :], bvecs
        )
        - exp_constants
    )
    diag_determinants_sqrt = np.sqrt(diagonal_determinants)
    diagonal_determinants_list = (
        diag_determinants_sqrt[row_indices] * diag_determinants_sqrt[col_indices]
    )
...
class WF:
    def __init__(
        ...
        self.num_dimensions = np.rint(0.5 * (-3 + sqrt(4 * len(nonlin_params[0]) + 9))).astype(
            int
        ...
        for i in range(self.num_gaussians):
            for j in range(0, i + 1):
                if abs(self.overlap[i, j]) > self.threshold_overlap:
                    ...
                    bvec = self.vectors[j].T @ self.matrices[j] + conj(
    ...
    def setupintermediates(self):
        ...
        self.matrices = np.einsum("ijk,ilk->ijl", self.L_matrices, self.L_matrices) + 1j * (
            self.K_matrices + np.einsum("ijk->ikj", self.K_matrices)
        )
        for i in range(ng):
            self.L_matrices[i, :, :][np.tril_indices(nd)] = self.nonlin_params[i][
                : int(0.5 * nd * (nd + 1))
            ]
            self.K_matrices[i, :, :][np.tril_indices(nd)] = self.nonlin_params[i][
        ...
        self.vmmips = np.einsum(
            "ia,iab,ib->i", self.vectors, self.matrices_squared, self.vectors
        )
        ...
        self.sum_matrices, self.sum_matrices_inv = make_sum_matrices(
            self.matrices, np.empty((ng, ng, nd, nd), dtype=complex), None
    ...
    def update_params_overlapFitting(
        self, indices, new_nonlin_params, new_lin_params=None, onlyX1X2=False
    ):
        """
        ...
        for i in range(num_new_gauss):
            new_L_matrices[i, :, :][np.tril_indices(nd)] = new_nonlin_params[i][
                : int(0.5 * nd * (nd + 1))
            ]
            new_K_matrices[i, :, :][np.tril_indices(nd)] = new_nonlin_params[i][
                int(0.5 * nd * (nd + 1)) : (nd * (nd + 1))
            ]
        ...
        self.sum_matrices, self.sum_matrices_inv = update_sum_matrices(
            self.matrices, pairs, self.sum_matrices, self.sum_matrices_inv
        )
        self.overlap = update_overlap_matrix(
            self.overlap,
            pairs,
            self.matrices,
            self.vectors,
            self.sum_matrices,
            self.sum_matrices_inv,
        )
        mu_list = []
        cross_mat_inv_list = []
        ovlp_list = []
        ...
        for i in range(ng):
            ...
            for a in range(nd):
                for b in range(a + 1):
                    ...
                    D_tensor[i, counter] += (
                        2 * q.T @ (-B @ Ainv @ Aderiv @ Ainv @ B + Aderiv) @ q
                    )
                    Bderiv = real(self.Kab[i, a, b])
                    D_tensor[i, counter + (nd * (nd + 1)) // 2] = 2 * (
                        q.T @ B @ Ainv @ Bderiv @ q + q.T @ Bderiv @ Ainv @ B @ q
                    )
                    counter += 1
    ...
    def calculate_overlap_matrix_normalized(self):
        ...
        diagonal_sum_matrix = np.diagonal(self.sum_matrices, axis1=0, axis2=1).transpose(
            [2, 0, 1]
        )  # Same as np.einsum('iijk->ijk', self.sum_matrices)
        ...
        self.overlap = np.einsum(
            "ij,i,j->ij", inv_dets, sqrt_diagonal_dets, sqrt_diagonal_dets
        )
        exponents = (
            np.einsum("ija,ijab,ijb->ij", bvec_outer, self.sum_matrices_inv, bvec_outer)
            - vmips_outer
            - 0.5 * diagonal_exponents_outer
        )
    ...
    def calculate_full_overlap_deriv_matrix(self, start_index_WFnew):
        ...
        D_tensor[:, :, : self.ab_iter] = -einsum(
            "ijkl,cjkl->ijc", self.x2_expectation_values[:, ngMs:], self.IabLj_list[:, ngMs:]
        )
        D_tensor[:, :, : self.ab_iter] += 2 * einsum(
            "ijk,cjk->ijc", self.x1_expectation_values[:, ngMs:], self.IabLjmuj_list[:, ngMs:]
        )
        D_tensor[:, :, : self.ab_iter] += -1 * einsum(
            "ij,jk,cjk->ijc",
            self.overlap[:, ngMs:],
            self.vectors[ngMs:],
            self.IabLjmuj_list[:, ngMs:],
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] = -1j * einsum(
            "ijab,cjab->ijc", self.x2_expectation_values[:, ngMs:], self.Kab_list[:, ngMs:]
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += 2j * einsum(
            "ijk,cjk->ijc", self.x1_expectation_values[:, ngMs:], self.Kabmuj_list[:, ngMs:]
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += -1j * einsum(
            "ij,jk,cjk->ijc",
            self.overlap[:, ngMs:],
            self.vectors[ngMs:],
            self.Kabmuj_list[:, ngMs:],
        )
        D_tensor_mu = -2 * einsum(
            "jak,jk,ij->ija",
            self.matrices[ngMs:, :, :],
            self.vectors[ngMs:, :],
            self.overlap[:, ngMs:],
    ...
    def calculate_potential(self):
        ...
        self.potential += (
            -self.lambda_
            / 3
            * einsum("ijkkk->ij", self.x3_expectation_values[:, :, 1:, 1:, 1:])
        )  # First type of third order terms
        self.potential += self.lambda_ * einsum(
            "ijkkk->ij", self.x3_expectation_values[:, :, 0:-1, 0:-1, 1:]
        )
    ...
    def calculate_full_potential_deriv_matrix(self, start_index_WFnew):
        ...
        x5_contr1 = einsum(
            "ijlmkkk->ijlm", self.x5_expectation_values[:, ngMs:, :, :, 1:, 1:, 1:]
        )
        x5_contr2 = einsum(
            "ijlmkkk->ijlm", self.x5_expectation_values[:, ngMs:, :, :, 0:-1, 0:-1, 1:]
        )
        x5_contr = self.lambda_ / 3 * x5_contr1 - self.lambda_ * x5_contr2
        x4_contr = einsum(
            "ijlkkk->ijl",
            -self.lambda_ / 3 * 2 * self.x4_expectation_values[:, ngMs:, :, 1:, 1:, 1:],
        )
        x4_contr += einsum(
            "ijlkkk->ijl",
            2 * self.lambda_ * self.x4_expectation_values[:, ngMs:, :, 0:-1, 0:-1, 1:],
        )
        ...
        D_tensor[:, :, : self.ab_iter] += einsum(
            "ijl,cjl->ijc", contr3, self.IabLjmuj_list[:, ngMs:]
        )
        D_tensor[:, :, : self.ab_iter] += einsum(
            "cjab,ijab->ijc", self.IabLj_list[:, ngMs:], contr2
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "ijl,cjl->ijc", contr3, self.Kabmuj_list[:, ngMs:]
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cjab,ijab->ijc", self.Kab_list[:, ngMs:], contr2
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] *= 1j

        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum(
            "jak,ijkll->ija",
            self.matrices[ngMs:, :, :],
            self.x3_expectation_values[:, ngMs:, :, :, :],
        )
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum(
            "jak,ijk->ija", self.matrices[ngMs:, :, :], x4_contr
        )
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += -2 * einsum(
            "jak,jk,ij->ija",
            self.matrices[ngMs:, :, :],
            self.vectors[ngMs:, :],
            potential[:, ngMs:],
    ...
    def calculate_kinetic_energy(self):
        ...
        self.kinetic += -2 * einsum(
            "ijab,jab->ij", self.x2_expectation_values, self.matrices_squared
        )
        return self.kinetic
    ...
    def calculate_full_kinetic_energy_deriv(self, start_index_WFnew):
        ...
        C1 += einsum("i,ij,jk->ijk", cis, self.overlap[:, ngMs:], self.vectors[ngMs:])
        C2 += 2 * einsum(
            "ijnmkl,imn->ijkl", self.x4_expectation_values[:, ngMs:], matrices_squared_conj
        )
        C1 += -4 * einsum(
            "ijnmk,imn->ijk", self.x3_expectation_values[:, ngMs:], matrices_squared_conj
        )
        ...
        D_tensor[:, :, : self.ab_iter] += einsum(
            "ijnm,cjnm->ijc", C2, self.IabLj_list[:, ngMs:]
        )
        D_tensor[:, :, : self.ab_iter] += einsum(
            "ijk,cjk->ijc", C1, self.IabLjmuj_list[:, ngMs:]
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "ijnm,cjnm->ijc", C2, self.Kab_list[:, ngMs:]
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "ijk,cjk->ijc", C1, self.Kabmuj_list[:, ngMs:]
        )
        ...
        D_tensor_mu = 2 * einsum(
            ...
            cis,
            self.matrices[ngMs:, :, :],
            self.x1_expectation_values[:, ngMs:, :],
        )
        D_tensor_mu += -2 * einsum(
            "ij,jaf,jf->ija", kinetic[:, ngMs:], self.matrices[ngMs:], self.vectors[ngMs:, :]
        )
        D_tensor_mu += 8 * einsum(
            "jaf,ig,ijfg->ija",
            self.matrices[ngMs:, :, :],
            self.rho_T_conjs,
            self.x2_expectation_values[:, ngMs:],
        )
        D_tensor_mu += -4 * einsum(
            "ifg,jak,ijfgk->ija",
            matrices_squared_conj,
            self.matrices[ngMs:, :, :],
            self.x3_expectation_values[:, ngMs:],
        )
    ...
    def calculate_kinetic_energy_squared(self):
        self.kinetic_energy_squared = np.empty(
            (self.num_gaussians, self.num_gaussians), dtype=np.complex128
        )
        cjs = self.cjs
        cis = self.cis
        rho_T_conjs = self.rho_T_conjs
        self.kinetic_energy_squared = -2 * einsum(
            "a,bij,abij->ab", cis, self.matrices_squared, self.x2_expectation_values
        )
        self.kinetic_energy_squared += -2 * einsum(
            "b,aij,abij->ab", cjs, self.matrices_squared_conj, self.x2_expectation_values
        )
        self.kinetic_energy_squared += 16 * einsum(
            "ai,bj,abij->ab", rho_T_conjs, self.omega_T, self.x2_expectation_values
        )
        self.kinetic_energy_squared += 4 * einsum(
            "a,bj,abj->ab", cis, self.omega_T, self.x1_expectation_values
        )
        self.kinetic_energy_squared += 4 * einsum(
            "b,aj,abj->ab", cjs, rho_T_conjs, self.x1_expectation_values
        )
        self.kinetic_energy_squared -= 8 * einsum(
            "bi,ajm,abijm->ab",
            self.omega_T,
            self.matrices_squared_conj,
            self.x3_expectation_values,
        )
        self.kinetic_energy_squared -= 8 * einsum(
            "ai,bjm,abijm->ab",
            rho_T_conjs,
            (self.matrices_squared),
            self.x3_expectation_values,
        )
    ...
    def calculate_full_kinetic_energy_squared_deriv(self, start_index_WFnew):
        ...
        x6_eAisqconjmat = einsum(
            "abklijmn,aij->abklmn", self.x6_expectation_values, matrices_squared_conj
        )
        x5_eAisqconjmat = einsum(
            "abkijmn,aij->abkmn", self.x5_expectation_values, matrices_squared_conj
        )
        tempAjsqx6_eAisqconj = -4 * einsum(
            "jkl,ijklmn->ijmn", self.matrices_squared, x6_eAisqconjmat
        )
        tempx5_eAjsqAisqconj = 8 * einsum(
            "abmkl,bkl->abm", x5_eAisqconjmat, self.matrices_squared
        )
        rho_T_conjs = self.rho_T_conjs
        Asquared_derivs_I = einsum("jabkx,jxl->jabkl", self.IabLj[ngMs:], self.matrices[ngMs:])
        Asquared_derivs_I += einsum(
            "jkx,jabxl->jabkl", self.matrices[ngMs:], self.IabLj[ngMs:]
        )
        ...
        cj_derivs_I += -2 * einsum(
            "jk,jabkl,jl->jab", self.vectors[ngMs:], Asquared_derivs_I, self.vectors[ngMs:]
        )
        cj_derivs_K = einsum("jabkk->jab", self.Kab[ngMs:], dtype=complex)
        cj_derivs_K += -2 * einsum(
            "jk,jabkl,jl->jab", self.vectors[ngMs:], Asquared_derivs_K, self.vectors[ngMs:]
        )
        ...
        temp1 += -2 * einsum(
            "ijkl,ikl->ij", self.x2_expectation_values[:, ngMs:], matrices_squared_conj
        )
        ...
        temp3 += -8 * einsum(
            "ilm,ijklm->ijk", matrices_squared_conj, self.x3_expectation_values[:, ngMs:]
        )
        temp4 = -2 * einsum("i,ijnm->ijnm", cis, self.x2_expectation_values[:, ngMs:])
        temp4 += -8 * einsum(
            "ik,ijnmk->ijnm", rho_T_conjs, self.x3_expectation_values[:, ngMs:]
        )
        temp4 += 4 * einsum(
            "ikl,ijnmkl->ijnm", matrices_squared_conj, self.x4_expectation_values[:, ngMs:]
        )

        temp2 = -einsum("j,i,ij->ij", cjs[ngMs:], cis, self.overlap[:, ngMs:])

        temp2 += -4 * einsum(
            "j,ijk,ik->ij", cjs[ngMs:], self.x1_expectation_values[:, ngMs:], rho_T_conjs
        )
        temp2 += -4 * einsum(
            "i,jk,ijk->ij", cis, self.omega_T[ngMs:], self.x1_expectation_values[:, ngMs:]
        )
        temp2 += 2 * einsum(
            "j,ikl,ijkl->ij",
            cjs[ngMs:],
        ...
        temp2 += -4 * einsum(
            "jkl,ijklmn,imn->ij",
            self.matrices_squared[ngMs:],
            self.x4_expectation_values[:, ngMs:],
            matrices_squared_conj,
        )

        temp5 = 2 * einsum(
            "j,i,ijm->ijm", cjs[ngMs:], cis, self.x1_expectation_values[:, ngMs:]
        )
        temp5 += 8 * einsum(
            "j,il,ijml->ijm", cjs[ngMs:], rho_T_conjs, self.x2_expectation_values[:, ngMs:]
        )
        temp5 += -4 * einsum(
            "j,ijmkl,ikl->ijm",
            cjs[ngMs:],
        ...
        temp5 += 8 * einsum(
            "i,jk,ijmk->ijm", cis, self.omega_T[ngMs:], self.x2_expectation_values[:, ngMs:]
        )
        temp5 += -4 * einsum(
            "i,jkl,ijmkl->ijm",
            cis,
        ...
        temp6 += 8 * einsum(
            "ijklmn,jkl->ijmn", x5rhomat[:, ngMs:], self.matrices_squared[ngMs:]
        )
        temp6 += tempAjsqx6_eAisqconj[:, ngMs:]
        temp6 += -einsum(
            "j,i,ijkl->ijkl", cjs[ngMs:], cis, self.x2_expectation_values[:, ngMs:]
        )
        temp6 += -4 * einsum(
            "j,ijlmn,il->ijmn", cjs[ngMs:], self.x3_expectation_values[:, ngMs:], rho_T_conjs
        )
        temp6 += -4 * einsum(
            "i,jk,ijkmn->ijmn", cis, self.omega_T[ngMs:], self.x3_expectation_values[:, ngMs:]
        )
        temp6 += 2 * einsum(
            "i,jkl,ijklmn->ijmn",
            cis,
        ...
        D_tensor[:, :, : self.ab_iter] += einsum(
            "cjnm,ijnm->ijc", Asquared_derivs_I_list[:, :], temp4
        )
        ...
        D_tensor[:, :, : self.ab_iter] += einsum(
            "cjm,ijm->ijc", self.IabLjmuj_list[:, ngMs:], temp5
        )
        D_tensor[:, :, : self.ab_iter] += einsum(
            "ijmn,cjnm->ijc", temp6, self.IabLj_list[:, ngMs:]
        )

        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cj,ij->ijc", mKm_list[:, ngMs:], temp2
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cjnm,ijnm->ijc", Asquared_derivs_K_list[:, :], temp4
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "ij,cj->ijc", temp1, cj_derivs_K_list[:, :]
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cjk,ijk->ijc", omega_derivs_K_list, temp3
        )

        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cjm,ijm->ijc", self.Kabmuj_list[:, ngMs:], temp5
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "ijmn,cjnm->ijc", temp6, self.Kab_list[:, ngMs:]
        )

        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] *= 1j

        temp7 = -8 * einsum(
            "j,ik,ijk->ij", cjs[ngMs:], rho_T_conjs, self.x1_expectation_values[:, ngMs:]
        )
        temp7 += 4 * einsum(
            "j,ifg,ijfg->ij",
            cjs[ngMs:],
        ...
        temp8 = 8 * einsum(
            "j,if,ijfn->ijn", cjs[ngMs:], rho_T_conjs, self.x2_expectation_values[:, ngMs:]
        )
        temp8 += -4 * einsum(
            "j,ifg,ijfgn->ijn",
            cjs[ngMs:],
        ...
        temp8 += 8 * einsum(
            "i,jk,ijkn->ijn", cis, self.omega_T[ngMs:], self.x2_expectation_values[:, ngMs:]
        )
        temp8 += -4 * einsum(
            "i,jfg,ijfgn->ijn",
            cis,
        ...
        temp8 += 2 * einsum(
            "j,i,ijn->ijn", cjs[ngMs:], cis, self.x1_expectation_values[:, ngMs:]
        )
        temp8 += -16 * einsum(
            "jk,ifg,ijknfg->ijn",
            self.omega_T[ngMs:],
        ...
        temp8 += 8 * einsum(
            "jkl,ijnkl->ijn", self.matrices_squared[ngMs:], x5_eAisqconjmat[:, ngMs:]
        )
        ...
        temp10 += 16 * einsum(
            "il,ijlk->ijk", rho_T_conjs, self.x2_expectation_values[:, ngMs:]
        )
        temp10 += -8 * einsum(
            "ilm,ijlmk->ijk", matrices_squared_conj, self.x3_expectation_values[:, ngMs:]
        )
        ...
        vaAsquareds = self.matrices_squared[
            ngMs:
        ]  # einsum("ak,jkl->jal",self.vas,self.matrices_squared[ngMs:])

        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum(
            "ijn,jan->ija", temp8, self.matrices[ngMs:]
        )
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum("ij,ja->ija", temp7, zs)
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum("ij,ja->ija", temp9, ck_derivs)
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum(
            "ijk,jak->ija", temp10, vaAsquareds
        )
        ...
        fodmd = self.fodmd
        D_tensor += -0.5 * np.einsum(
            "ij,jk->ijk", self.kinetic_energy_squared[:, ngMs:], fodmd[ngMs:, :]
        )
        return D_tensor
    ...
    def calculate_potential_times_kinetic_energy(self):
        ...
        self.potential_times_kinetic = (
            ...
            * einsum(
                "abmmmij,bij->ab",
                self.x5_expectation_values[:, :, 1:, 1:, 1:, :, :],
                self.matrices_squared,
            )
        )
        self.potential_times_kinetic += (
        ...
        self.potential_times_kinetic += (
            ...
            * self.lambda_
            * einsum(
                "abijjj,bi->ab", self.x4_expectation_values[:, :, :, 1:, 1:, 1:], self.omega_T
            )
        )
        self.potential_times_kinetic += (
            4
            ...
            * einsum(
                "abijjj,bi->ab",
                self.x4_expectation_values[:, :, :, :-1, :-1, 1:],
                self.omega_T,
            )
        )
        self.potential_times_kinetic += -einsum(
            "abijmm,bij->ab", self.x4_expectation_values, self.matrices_squared
        )
        self.potential_times_kinetic += (
            -1
            / 3
            * self.lambda_
            * einsum("b,abiii->ab", cj, self.x3_expectation_values[:, :, 1:, 1:, 1:])
        )
        self.potential_times_kinetic += self.lambda_ * einsum(
            "b,abiii->ab", cj, self.x3_expectation_values[:, :, 0:-1, 0:-1, 1:]
        )
        self.potential_times_kinetic += 0.5 * np.einsum(
            "b,abii->ab", cj, self.x2_expectation_values
        )
        self.potential_times_kinetic += 2 * einsum(
            "bi,abijj->ab", self.omega_T, self.x3_expectation_values
        )
    ...
    def calculate_full_potential_times_kinetic_deriv(self, start_index_WFnew):
        ...
        x5e_c1_mat = einsum(
            "abijmmm->abij", self.x5_expectation_values[:, ngMs:, :, :, :-1, :-1, 1:]
        )
        x5e_c2_mat = einsum(
            "abijmmm->abij", self.x5_expectation_values[:, ngMs:, :, :, 1:, 1:, 1:]
        )
        ...
        x4e_cV1_mat = einsum(
            "abijjj->abi", self.x4_expectation_values[:, ngMs:, :, :-1, :-1, 1:]
        )
        x4e_cV2_mat = einsum(
            "abijjj->abi", self.x4_expectation_values[:, ngMs:, :, 1:, 1:, 1:]
        )
        ...
        Asquared_derivs_I = einsum("jabkx,jxl->jabkl", self.IabLj[ngMs:], self.matrices[ngMs:])
        Asquared_derivs_I += einsum(
            "jabxl,jkx->jabkl", self.IabLj[ngMs:], self.matrices[ngMs:]
        )
        Asquared_derivs_K = einsum("jabkx,jxl->jabkl", self.Kab[ngMs:], self.matrices[ngMs:])
        Asquared_derivs_K += einsum("jkx,jabxl->jabkl", self.matrices[ngMs:], self.Kab[ngMs:])
        ...
        temp_contr7 = (
            +2 * self.lambda_ * self.x7_c3[:, ngMs:]
            - 2 / 3 * self.lambda_ * self.x7_c4[:, ngMs:]
        )
        ...
        temp1 += einsum(
            "imn,ijmnkk->ij", matrices_squared_conj, self.x4_expectation_values[:, ngMs:]
        )
        temp1 += einsum("jk,ijk->ij", self.omega_T[ngMs:], -2 * temp_contr4)
        temp1 += -2 * einsum("jkl,ijkl->ij", self.matrices_squared[ngMs:], temp_contr5)
        temp1 += -2 * einsum(
            "jk,ijkll->ij", self.omega_T[ngMs:], self.x3_expectation_values[:, ngMs:]
        )
        temp1 += einsum(
            "jmn,ijmnkk->ij",
            self.matrices_squared[ngMs:],
        ...
        temp1 += einsum(
            "i,ij->ij",
            cis,
            -self.lambda_ * x3e_c1_mat + 1 / 3 * self.lambda_ * x3e_c2_mat - 0.5 * x2e_c_mat,
        )
        temp1 += -einsum("ij->ij", 4 * self.lambda_ * x4e_c1_mat)
        temp1 += einsum(
            "j,ij->ij",
            cjs[ngMs:],
            +self.lambda_ * 1 / 3 * x3e_c2_mat - 0.5 * x2e_c_mat - self.lambda_ * x3e_c1_mat,
        )
        ...
        temp2 += 4 * einsum(
            "ik,ijmkll->ijm", rho_T_conjs, self.x4_expectation_values[:, ngMs:]
        )
        temp2 += -2 * einsum("ikl,ijmkl->ijm", matrices_squared_conj, x5_e_c3_mat)
        temp2 += 4 * einsum(
            "jk,ijmkll->ijm", self.omega_T[ngMs:], self.x4_expectation_values[:, ngMs:]
        )
        ...
        temp3 += -0.5 * einsum(
            "j,ijklxx->ijkl", cjs[ngMs:], self.x4_expectation_values[:, ngMs:]
        )
        ...
        D_tensor[:, :, : self.ab_iter] += einsum(
            "cjm,ijm->ijc", self.IabLjmuj_list[:, ngMs:], temp2
        )
        D_tensor[:, :, : self.ab_iter] += einsum(
            "cjkl,ijkl->ijc", self.IabLj_list[:, ngMs:], temp3
        )
        D_tensor[:, :, : self.ab_iter] += einsum(
            "cjk,ijk->ijc", omega_derivs_I_list[:, :], temp4
        )
        D_tensor[:, :, : self.ab_iter] += einsum(
            "cjkl,ijkl->ijc", Asquared_derivs_I_list[:, :], temp5
        )
        D_tensor[:, :, : self.ab_iter] += einsum("cj,ij->ijc", cj_derivs_I_list[:, :], temp6)

        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cj,ij->ijc", mKm_list[:, ngMs:], temp1
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cjm,ijm->ijc", self.Kabmuj_list[:, ngMs:], temp2
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cjkl,ijkl->ijc", self.Kab_list[:, ngMs:], temp3
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cjk,ijk->ijc", omega_derivs_K_list[:, :], temp4
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cjkl,ijkl->ijc", Asquared_derivs_K_list[:, :], temp5
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cj,ij->ijc", cj_derivs_K_list[:, :], temp6
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] *= 1j

        omega_derivs = self.matrices_squared[
            ngMs:
        ]  # einsum("ak,jlk->jal",self.vas,self.matrices_squared[ngMs:])
        ...
        temp7 += 4 * einsum(
            "jx,ijxfkk->ijf", self.omega_T[ngMs:], self.x4_expectation_values[:, ngMs:]
        )
        temp7 += -8 * einsum("ix,ijxf->ijf", rho_T_conjs, temp_contr5)
        temp7 += -8 * einsum("jx,ijxf->ijf", self.omega_T[ngMs:], temp_contr5)
        temp7 += 4 * einsum(
            "ix,ijxfkk->ijf", rho_T_conjs, self.x4_expectation_values[:, ngMs:]
        )
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum(
            "ijf,jaf->ija", temp7, self.matrices[ngMs:]
        )
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += 2 * einsum(
            "jaf,ijf->ija", omega_derivs, temp_contr4
        )
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += 2 * einsum(
            "jaf,ijfgg->ija", omega_derivs, self.x3_expectation_values[:, ngMs:]
        )

        temp8 = -self.lambda_ * 2 * einsum("j,ij->ij", cjs[ngMs:], x3e_c1_mat)
        temp8 += self.lambda_ * 2 / 3 * einsum("j,ij->ij", cjs[ngMs:], x3e_c2_mat)
        temp8 += 2 * einsum(
            "ifg,ijfgkk->ij", matrices_squared_conj, self.x4_expectation_values[:, ngMs:]
        )
        ...
        temp8 += -4 * einsum(
            "jf,ijfgg->ij", self.omega_T[ngMs:], self.x3_expectation_values[:, ngMs:]
        )
        temp8 += +2 * einsum(
            "jfg,ijfgkk->ij",
            self.matrices_squared[ngMs:],
        ...
        fodmd = self.fodmd
        D_tensor += -0.5 * np.einsum(
            "ij,jk->ijk", self.potential_times_kinetic[:, ngMs:], fodmd[ngMs:, :]
        )
        return D_tensor

    def calculate_potential_squared(self):
        self.potential_squared = 0.25 * einsum("abiijj->ab", self.x4_expectation_values)
        self.potential_squared += self.lambda_ * einsum(
            "abiijjj->ab", self.x5_expectation_values[:, :, :, :, :-1, :-1, 1:]
        )
        self.potential_squared += (
            -self.lambda_
            / 3
            * einsum("abiijjj->ab", self.x5_expectation_values[:, :, :, :, 1:, 1:, 1:])
        )
        self.potential_squared += (
            self.lambda_sq
            / 9
            * einsum("abiiijjj->ab", self.x6_expectation_values[:, :, 1:, 1:, 1:, 1:, 1:, 1:])
        )
        self.potential_squared += self.lambda_sq * einsum(
            "abiiijjj->ab", self.x6_expectation_values[:, :, :-1, :-1, 1:, :-1, :-1, 1:]
        )
        self.potential_squared += (
            -2
            / 3
            * self.lambda_sq
            * einsum(
                "abiiijjj->ab", self.x6_expectation_values[:, :, 1:, 1:, 1:, 0:-1, 0:-1, 1:]
            )
        )
    ...
    def calculate_full_potential_squared_deriv_matrix(self, start_index_WFnew):
        ...
        x6_e_contracted_full = einsum(
            "abkliijj->abkl", self.x6_expectation_values[:, ngMs:]
        )  # Candidate for wrongness (only enters super_contr2)
        x6_e_contracted_firstV = einsum(
            "abkiijjj->abk", self.x6_expectation_values[:, ngMs:, :, :, :, :-1, :-1, 1:]
        )
        x6_e_contracted_secondV = einsum(
            "abkiijjj->abk", self.x6_expectation_values[:, ngMs:, :, :, :, 1:, 1:, 1:]
        )
        super_contr1 = (
        ...
        super_contr1 += (
            2 / 9 * self.lambda_sq * self.x7_c1v[:, ngMs:]
            + 2 * self.lambda_sq * self.x7_c2v[:, ngMs:]
            + -4 / 3 * self.lambda_sq * self.x7_c3v[:, ngMs:]
        )
        ...
        D_tensor[:, :, : self.ab_iter] += einsum(
            "cjkl,ijkl->ijc", self.IabLj_list[:, ngMs:], super_contr2
        )
        D_tensor[:, :, : self.ab_iter] += einsum(
            "cjk,ijk->ijc", self.IabLjmuj_list[:, ngMs:], super_contr1
        )
        D_tensor[:, :, : self.ab_iter] += -einsum(
            "ij,jk,cjk->ijc",
            potentialsquared[:, ngMs:],
            self.vectors[ngMs:],
            self.IabLjmuj_list[:, ngMs:],
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cjkl,ijkl->ijc", self.Kab_list[:, ngMs:], super_contr2
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += einsum(
            "cjk,ijk->ijc", self.Kabmuj_list[:, ngMs:], super_contr1
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += -einsum(
            "ij,jk,cjk->ijc",
            potentialsquared[:, ngMs:],
        ...
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] *= 1j
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum(
            "jck,ijk->ijc", self.matrices[ngMs:, :], super_contr1
        )
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += -2 * einsum(
            "ij,jck,jk->ijc",
            potentialsquared[:, ngMs:],
        ...
        fodmd = self.fodmd
        D_tensor += -0.5 * np.einsum(
            "ij,jk->ijk", self.potential_squared[:, ngMs:], fodmd[ngMs:, :]
        )
    ...
    def calculate_Hamiltonian_deriv(self, start_index_WFnew):
        return self.calculate_full_potential_deriv_matrix(
            start_index_WFnew
        ) + self.calculate_full_kinetic_energy_deriv(start_index_WFnew)
    ...
    def calculate_Hamiltonian_squared_deriv(self, start_index_WFnew):
        returnval = self.calculate_full_kinetic_energy_squared_deriv(
            start_index_WFnew
        )  # Takes second most time
        returnval += self.calculate_full_potential_squared_deriv_matrix(start_index_WFnew)
        returnval += self.calculate_full_potential_times_kinetic_deriv(
            start_index_WFnew
        )  # Takes most time
        return returnval
    ...
    def calculate_euler_error(self, start_index_WFnew, h):
        ...
        cross_terms = -2 * (
            np.conj(new_lin_params).T @ (O - h * 1j * H)[si:, :si] @ old_lin_params
        )
    ...
    def rothe_error(self, start_index_WFnew, h):
        ...
        self.rothe_optimal_c_normalization(start_index_WFnew, h)
        projection_term = (
            2 * conj(self.rho).T @ self.opt_c - conj(self.opt_c).T @ self.Smat @ self.opt_c
        )
