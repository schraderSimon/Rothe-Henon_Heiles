...
def update_overlap_matrix(overlap_matrix, pairs_of_indices, matrices, vectors, sum_matrices, sum_matrices_inv):
    ...
    diagonal_exponents = (
        np.einsum("ak,akl,al->a", bvec_calc, diagonal_sum_matrices_inv, bvec_calc) - vmips - vmips.conj()
    )
    ...
    exponents = (
        np.einsum("ia,iab,ib->i", bvecs, sum_matrices_inv[row_indices, col_indices, :, :], bvecs) - exp_constants
    )
...
class WF:
    ...
    def update_params_overlapFitting(self, indices, new_nonlin_params, new_lin_params=None, onlyX1X2=False):
        ...
        self.overlap = update_overlap_matrix(
            self.overlap, pairs, self.matrices, self.vectors, self.sum_matrices, self.sum_matrices_inv
        )
        mu_list = []
        cross_mat_inv_list = []
        ovlp_list = []
    ...
    def calculate_full_overlap_deriv_matrix(self, start_index_WFnew):
        ...
        D_tensor[:, :, : self.ab_iter] += -1 * einsum(
            "ij,jk,cjk->ijc", self.overlap[:, ngMs:], self.vectors[ngMs:], self.IabLjmuj_list[:, ngMs:]
        )
        ...
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += -1j * einsum(
            "ij,jk,cjk->ijc", self.overlap[:, ngMs:], self.vectors[ngMs:], self.Kabmuj_list[:, ngMs:]
        )
        D_tensor_mu = -2 * einsum(
            "jak,jk,ij->ija", self.matrices[ngMs:, :, :], self.vectors[ngMs:, :], self.overlap[:, ngMs:]
        )
        D_tensor_mu += 2 * einsum(
            "jak,ijk->ija",
            self.matrices[ngMs:, :, :],
    ...
    def calculate_potential(self):
        ...
        self.potential += (
            -self.lambda_ / 3 * einsum("ijkkk->ij", self.x3_expectation_values[:, :, 1:, 1:, 1:])
    ...
    def calculate_full_potential_deriv_matrix(self, start_index_WFnew):
        ...
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum(
            "jak,ijkll->ija", self.matrices[ngMs:, :, :], self.x3_expectation_values[:, ngMs:, :, :, :]
        )
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum("jak,ijk->ija", self.matrices[ngMs:, :, :], x4_contr)
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += -2 * einsum(
            "jak,jk,ij->ija", self.matrices[ngMs:, :, :], self.vectors[ngMs:, :], potential[:, ngMs:]
        )
    ...
    def calculate_full_kinetic_energy_deriv(self, start_index_WFnew):
        ...
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] *= 1j
        D_tensor_mu = 2 * einsum(
            "i,jaf,ijf->ija", cis, self.matrices[ngMs:, :, :], self.x1_expectation_values[:, ngMs:, :]
        )
        D_tensor_mu += -2 * einsum("ij,jaf,jf->ija", kinetic[:, ngMs:], self.matrices[ngMs:], self.vectors[ngMs:, :])
        D_tensor_mu += 8 * einsum(
            "jaf,ig,ijfg->ija", self.matrices[ngMs:, :, :], self.rho_T_conjs, self.x2_expectation_values[:, ngMs:]
        )
    ...
    def calculate_kinetic_energy_squared(self):
        ...
        self.kinetic_energy_squared -= 8 * einsum(
            "bi,ajm,abijm->ab", self.omega_T, self.matrices_squared_conj, self.x3_expectation_values
        )
        self.kinetic_energy_squared -= 8 * einsum(
            "ai,bjm,abijm->ab", rho_T_conjs, (self.matrices_squared), self.x3_expectation_values
        )

        self.kinetic_energy_squared += einsum("a,b,ab->ab", cis, cjs, self.overlap)
        self.kinetic_energy_squared += 4 * einsum(
            "aij,bmn,abijmn->ab", self.matrices_squared_conj, self.matrices_squared, self.x4_expectation_values
        )
    ...
    def calculate_full_kinetic_energy_squared_deriv(self, start_index_WFnew):
        ...
        temp2 += 8 * einsum(
            "jk,ijklm,ilm->ij", self.omega_T[ngMs:], self.x3_expectation_values[:, ngMs:], matrices_squared_conj
        )
        temp2 += 8 * einsum(
            "ijmkl,im,jkl->ij", self.x3_expectation_values[:, ngMs:], rho_T_conjs, self.matrices_squared[ngMs:]
        )
        temp2 += -4 * einsum(
            "jkl,ijklmn,imn->ij",
        ...
        temp5 += -4 * einsum(
            "j,ijmkl,ikl->ijm", cjs[ngMs:], self.x3_expectation_values[:, ngMs:], matrices_squared_conj
        )
        temp5 += 8 * einsum("i,jk,ijmk->ijm", cis, self.omega_T[ngMs:], self.x2_expectation_values[:, ngMs:])
        temp5 += -4 * einsum(
            "i,jkl,ijmkl->ijm", cis, self.matrices_squared[ngMs:], self.x3_expectation_values[:, ngMs:]
        )
        temp5 += 32 * einsum("ijmkl,jk,il->ijm", self.x3_expectation_values[:, ngMs:], self.omega_T[ngMs:], rho_T_conjs)
        temp5 += -16 * einsum(
            "ijmkln,jk,iln->ijm", self.x4_expectation_values[:, ngMs:], self.omega_T[ngMs:], matrices_squared_conj
        )
        temp5 += -16 * einsum(
            "ijmnkl,in,jkl->ijm", self.x4_expectation_values[:, ngMs:], rho_T_conjs, self.matrices_squared[ngMs:]
        )
        temp5 += tempx5_eAjsqAisqconj[:, ngMs:]

        temp6 = 2 * einsum(
            "j,ikl,ijklmn->ijmn", cjs[ngMs:], matrices_squared_conj, self.x4_expectation_values[:, ngMs:]
        )
        temp6 += -16 * einsum(
            "jk,ijklmn,il->ijmn", self.omega_T[ngMs:], self.x4_expectation_values[:, ngMs:], rho_T_conjs
        )
        ...
        temp6 += 2 * einsum(
            "i,jkl,ijklmn->ijmn", cis, self.matrices_squared[ngMs:], self.x4_expectation_values[:, ngMs:]
        )
        ...
        temp7 += 16 * einsum(
            "jk,ifg,ijkfg->ij", self.omega_T[ngMs:], matrices_squared_conj, self.x3_expectation_values[:, ngMs:]
        )
        temp7 += 16 * einsum(
            "if,jkl,ijfkl->ij", rho_T_conjs, self.matrices_squared[ngMs:], self.x3_expectation_values[:, ngMs:]
        )
        temp7 += -8 * einsum(
            "ikl,jfg,ijfgkl->ij",
            matrices_squared_conj,
        ...
        temp8 += -4 * einsum(
            "j,ifg,ijfgn->ijn", cjs[ngMs:], matrices_squared_conj, self.x3_expectation_values[:, ngMs:]
        )
        temp8 += 8 * einsum("i,jk,ijkn->ijn", cis, self.omega_T[ngMs:], self.x2_expectation_values[:, ngMs:])
        temp8 += -4 * einsum(
            "i,jfg,ijfgn->ijn", cis, self.matrices_squared[ngMs:], self.x3_expectation_values[:, ngMs:]
        )
        temp8 += 2 * einsum("j,i,ijn->ijn", cjs[ngMs:], cis, self.x1_expectation_values[:, ngMs:])
        temp8 += -16 * einsum(
            "jk,ifg,ijknfg->ijn", self.omega_T[ngMs:], matrices_squared_conj, self.x4_expectation_values[:, ngMs:]
        )
        temp8 += 32 * einsum("jk,ig,ijkgn->ijn", self.omega_T[ngMs:], rho_T_conjs, self.x3_expectation_values[:, ngMs:])
        temp8 += -16 * einsum(
            "if,jkl,ijfnkl->ijn", rho_T_conjs, self.matrices_squared[ngMs:], self.x4_expectation_values[:, ngMs:]
        )
    ...
    def calculate_potential_times_kinetic_energy(self):
        ...
        self.potential_times_kinetic = (
            ...
            / 3
            * self.lambda_
            * einsum("abmmmij,bij->ab", self.x5_expectation_values[:, :, 1:, 1:, 1:, :, :], self.matrices_squared)
        )
        self.potential_times_kinetic += (
            -2
            * self.lambda_
        ...
        self.potential_times_kinetic += (
            ...
            / 3
            * self.lambda_
            * einsum("abijjj,bi->ab", self.x4_expectation_values[:, :, :, 1:, 1:, 1:], self.omega_T)
        )
        self.potential_times_kinetic += (
            4 * self.lambda_ * einsum("abijjj,bi->ab", self.x4_expectation_values[:, :, :, :-1, :-1, 1:], self.omega_T)
        )
        self.potential_times_kinetic += -einsum("abijmm,bij->ab", self.x4_expectation_values, self.matrices_squared)
        self.potential_times_kinetic += (
            -1 / 3 * self.lambda_ * einsum("b,abiii->ab", cj, self.x3_expectation_values[:, :, 1:, 1:, 1:])
        )
    ...
    def calculate_full_potential_times_kinetic_deriv(self, start_index_WFnew):
        ...
        temp1 += einsum(
            "i,ij->ij", cis, -self.lambda_ * x3e_c1_mat + 1 / 3 * self.lambda_ * x3e_c2_mat - 0.5 * x2e_c_mat
        )
        temp1 += -einsum("ij->ij", 4 * self.lambda_ * x4e_c1_mat)
        temp1 += einsum(
            "j,ij->ij", cjs[ngMs:], +self.lambda_ * 1 / 3 * x3e_c2_mat - 0.5 * x2e_c_mat - self.lambda_ * x3e_c1_mat
        )
    ...
    def calculate_potential_squared(self):
        ...
        self.potential_squared += (
            -self.lambda_ / 3 * einsum("abiijjj->ab", self.x5_expectation_values[:, :, :, :, 1:, 1:, 1:])
        )
        self.potential_squared += (
            self.lambda_sq / 9 * einsum("abiiijjj->ab", self.x6_expectation_values[:, :, 1:, 1:, 1:, 1:, 1:, 1:])
        )
        ...
        self.potential_squared += (
            ...
            / 3
            * self.lambda_sq
            * einsum("abiiijjj->ab", self.x6_expectation_values[:, :, 1:, 1:, 1:, 0:-1, 0:-1, 1:])
        )
    ...
    def calculate_full_potential_squared_deriv_matrix(self, start_index_WFnew):
        ...
        D_tensor[:, :, : self.ab_iter] += -einsum(
            "ij,jk,cjk->ijc", potentialsquared[:, ngMs:], self.vectors[ngMs:], self.IabLjmuj_list[:, ngMs:]
        )
        ...
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] += -einsum(
            "ij,jk,cjk->ijc", potentialsquared[:, ngMs:], self.vectors[ngMs:], self.Kabmuj_list[:, ngMs:]
        )
        D_tensor[:, :, self.ab_iter : 2 * self.ab_iter] *= 1j
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += einsum("jck,ijk->ijc", self.matrices[ngMs:, :], super_contr1)
        D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)] += -2 * einsum(
            "ij,jck,jk->ijc", potentialsquared[:, ngMs:], self.matrices[ngMs:, :], self.vectors[ngMs:]
        )
        D_tensor[:, :, nd * (nd + 2) : nd * (nd + 3)] = 1j * D_tensor[:, :, nd * (nd + 1) : nd * (nd + 2)]
        fodmd = self.fodmd
    ...
    def calculate_Hamiltonian_deriv(self, start_index_WFnew):
        return self.calculate_full_potential_deriv_matrix(start_index_WFnew) + self.calculate_full_kinetic_energy_deriv(
            start_index_WFnew
        )
            ...
            for j in range(ngsi):  # For each Gaussian
                for a in range(self.num_dimensions * (self.num_dimensions + 3)):  # For each parameter
                    ...
                    def find_x(x):
                        ...
                        return real(
                            np.conj(c_deriv_cand).T @ ovlp_mat @ c
                            + np.conj(c) @ ovlp_mat @ c_deriv_cand
                            + np.conj(c) @ ovlp_deriv_mat @ c
                        )
                    ...
                    jac[j, a] = (
                        np.conj(c_deriv).T @ S @ c
                        + np.conj(c).T @ S @ c_deriv
                        + np.conj(c) @ S_deriv_mat @ c
                        - 2 * (np.conj(c_deriv) @ rho + np.conj(c).T @ rho_deriv_vec)
                    )
...
def test_derivs(typeInd=0, parInd=0, dim=3, lambda_=0.1):
    ...
    if type == types[4]:
        numerical = (
            dwf.calculate_potential_times_kinetic_energy()[0, 1] - wf.calculate_potential_times_kinetic_energy()[0, 1]
        )
    if type == types[3]:
