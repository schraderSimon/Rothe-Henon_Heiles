...
def transform_ECG_heller(ECG_A,ECG_B,ECG_mu,ECG_p,ECG_c):
    ...
    ECG_normalization_log=(ECG_p.T@ECG_B@invA@ECG_B@ECG_p+ECG_p.T@ECG_A@ECG_p) + d/4*log(pi/2)-0.25*log(np.linalg.det(ECG_A))
    heller_zeta=-1j*(np.log(ECG_c)-ECG_normalization_log+1j*heller_p.T@heller_q-heller_q.T@D@heller_q+b@Dinv@b)
...
def transform_heller_ECG(heller_C,heller_q,heller_p,heller_zeta):
    ...
    ECG_p=mu.imag
    ECG_normalization_log=(ECG_p.T@ECG_B@invA@ECG_B@ECG_p+ECG_p.T@ECG_A@ECG_p) + d/4*log(pi/2)-0.25*log(np.linalg.det(ECG_A))
    ECG_c_unnormalized=np.exp(1j*heller_zeta-1j*heller_p.T@heller_q+heller_q.T@D@heller_q-b@D_inv@b)
...
def propagate_kinetic_analytical(nonlin_params,lin_params,h):
    ...
    for i in range(ng):
        ...
        heller_C_new,heller_q_new,heller_p_new,heller_zeta_new=propagate_kinetic(heller_C,heller_q,heller_p,heller_zeta,h)
        A_new,B_new,mu_ECG_new,p_ECG_new,c_new=transform_heller_ECG(heller_C_new,heller_q_new,heller_p_new,heller_zeta_new)
...
if __name__=="__main__":
    ...
    heller_C,heller_q,heller_p,heller_zeta=transform_ECG_heller_fullyUnnormalized(ECG_A,ECG_B,ECG_mu,ECG_p,ECG_c)
    ECG_A_new,ECG_B_new,ECG_mu_new,ECG_p_new,ECG_c_new=transform_heller_ECG(heller_C,heller_q,heller_p,heller_zeta)
