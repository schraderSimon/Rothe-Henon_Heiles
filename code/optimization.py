...
class TimeEvolution():
    def __init__(self,wave_function,h,T,epsilon,t0,error_t0,filename,dimension,max_num_Gaussians=20):

        self.mask_lincoeff=[0.8865587802866912, -13.91445103929891, 78.58424070320325, -105.23506996256583, -5.7783020569168, 93.33308512709664, -22.677314700584247, -71.12872075341511, 53.47552955091851, 25.68122441131635, -65.63471326554463, 42.15941658394149, 8.056986128893868, -45.845105790207526, 56.248886712777676, -45.14073539380479, 26.356604035710916, -11.018158140453579, 2.985107837486339, -0.395107357239624]
        self.mask_nonlincoeff=[0.0067379471029344745, 0.008766285770907432, 0.011405219508509397, 0.014838556721848828, 0.01930543876899837, 0.025116996050942624, 0.032678018901526475, 0.04251515254382658, 0.05531357941370039, 0.07196474390737227, 0.09362844384567327, 0.12181361360477637, 0.15848342544522337, 0.20619202963199804, 0.26826245756591804, 0.34901807946651336, 0.4540837384217124, 0.5907775357109573, 0.7686205562022081, 1.0000000374130311]
        ...
        if not os.path.isfile(filename):
            ...
            with h5py.File(filename, "r+") as data_file:
                ...
                if abs(times_read[index_to_delete_after-1]-t0)>1e-5:
                    ...
                    del data_file["rothe_error"]; del data_file["times"]
    ...
    def rothe_gradient_numerical(self,nonlin_params):
        ...
        for i in range(len(nonlin_params)):
            ...
            new_nonlin[i]+=1e-7
            full_nonlin=np.concatenate((np.array(self.params_old_nonlin).flatten(),new_nonlin))
            full_nonlin=np.reshape(full_nonlin,(-1,n*(n+3)))
            full_lin=np.concatenate((self.params_old_lin,np.zeros(num_basis_functions)))
            optimization_WF=WF(full_nonlin,full_lin,lambda_=self.lambda_) 
            optimization_WF
    ...
    def rothe_error(self,nonlin_params, calculate_Gradient=True):
        ...
        optimization_WF=WF(full_nonlin,full_lin,lambda_=self.lambda_,calculate_Gradient=calculate_Gradient)
        self.WF_opt=optimization_WF
        RE=optimization_WF.rothe_error(start_index_WFnew=start_index,h=self.h)
        return RE
    def rothe_error_update(self,nonlin_params):
        ...
        start_index=len(self.params_old_lin)
        self.WF_opt.update_params_overlapFitting(indices,np.reshape(nonlin_params,(-1,self.dimension*(self.dimension+3))))
    ...
    def calculate_mask_coefficients(self):
        ...
        for i in range(num_basis_functions):
            ...
            for j in range(len(self.mask_nonlincoeff)):
                ...
                change_in_c=np.exp(-old_shift.T@old_at_old+old_at_old.T@newmat_inv@old_at_old-0.5*diagonal_exponent+0.5*diagonal_exponentN+0.5*log(normalization_new)-0.5*log(normalization_old))
    ...
    def make_mask_function(self,nonlin_params):
        ...
        optimization_WF=WF(full_nonlin,full_lin,lambda_=self.lambda_,calculate_Gradient=False,h=0,onlyX1X2=True)
    ...
    def rothe_error_mask_update(self,nonlin_params,onlyX1X2=True):
        ...
        self.WF_opt.update_params_overlapFitting(indices,np.reshape(nonlin_params,(-1,self.dimension*(self.dimension+3))),onlyX1X2=onlyX1X2)
        RE=self.WF_opt.rothe_error(start_index_WFnew=start_index,h=0)
        print("%e"%sqrt(RE))
        self.WF_opt.rothe_optimal_c_normalization(start_index,h=0)
    ...
    def euler_gradient_numerical(self,nonlin_params):
        ...
        for i in range(len(nonlin_params)):
            ...
            optimization_WF=WF(full_nonlin,full_lin,lambda_=self.lambda_) 
            optimization_WF
            REh=optimization_WF.calculate_euler_error(start_index_WFnew=start_index,h=self.h,calculate_Gradient=False)
            jac[i]=1/1e-7*(REh-RE)
        return jac

    def minimize_transformed_bonds(self,error_function,gradient,maxiter,start_params,args,gtol=0,multi_bonds=5e-1,printx=False):
        """
        ...
        def callback_func(xval):
            if self.numiter%10==0:
                ...
                if self.numiter>=20:
                    if sqrt(self.f_storage[-1]/self.f_storage[-2])>0.999 and self.f_storage[-1]/self.f_storage[-2]<1:
            ...
            self.numiter+=1
        try:
            self.transformed_sol,minval,gopt,bopt,nit,grad_calls,warnflag=fmin_bfgs(f=transformed_error,x0=transformed_params,fprime=transformed_gradient,
                        maxiter=200,gtol=1e-8,hess_inv0=hess_inv0,c1=1e-5,c2=3e-1,callback=callback_func,full_output=True)
    ...
    def calculate_importance_remove(self,nonlin_coefficients):
        ...
        n=self.dimension
        orig_RE=sqrt(self.rothe_error(nonlin_coefficients.flatten(),calculate_Gradient=False))
        ...
        for i in range(num_basis_functions_orig):
            ...
            nonlin_params_removed_indices=list(range(i))+list(range(i+1,num_basis_functions_orig))
            nonlin_params_removed=nonlin_coefficients.copy()[nonlin_params_removed_indices,:]
            r_errors[i]=self.rothe_error(nonlin_params_removed.flatten(),calculate_Gradient=False)
        for i in range(num_basis_functions_orig):
        ...
        if sqrt(r_errors[best])>10*orig_RE:
            ...
            return nonlin_coefficients.flatten()
        nonlin_params_removed_indices=list(range(best))+list(range(best+1,num_basis_functions_orig))
        nonlin_coefficients_new=nonlin_coefficients.copy()
        #nonlin_coefficients_new[best,:]=[sqrt(0.5),0.,sqrt(0.5),0,0,0,0,0,0,0]
        nonlin_coefficients_new=np.array(nonlin_coefficients[nonlin_params_removed_indices,:])
        startparams=np.array(nonlin_coefficients_new).flatten()#+(0.5-np.random.rand(len(nonlin_coefficients_new.flatten())))*1e-5
        ...
        solver=minimize(self.rothe_error,startparams,method="BFGS",jac=self.rothe_gradient,
    ...
    def add_gaussian(self,nonlin_coefficients):
        ...
        nonlin_coefficients_new=np.array(list(nonlin_coefficients.flatten())+list(optimal_params))
        startparams=np.array(nonlin_coefficients_new).flatten()+(np.random.rand(len(nonlin_coefficients_new.flatten()))-0.5)*1e-4
    ...
    def time_evolve(self):
        ...
        if run_euler:
            ...
            start=time.time()                                                   
            solver=minimize(self.euler_error,nonlin_params_initial,method="BFGS",jac=self.euler_gradient,
        ...
        if self.removed_at_prev_timestep>=2 and self.t>=0.2:
            best_nonlin_params, niter=self.minimize_transformed_bonds(opt_func,
                                                                self.rothe_gradient,
                                                                maxiter=maxiter,
            ...
            hess_inv0=np.diag(1/abs(grad0+1e-15*np.array(len(grad0))))              
            start=time.time()                                    
            solver = minimize(opt_func, nonlin_params_initial, method="BFGS", jac=self.rothe_gradient,
        ...
        if niter<=3:
            ...
            start=time.time()                                    
            solver = minimize(opt_func, pre_euler_params, method="BFGS", jac=self.rothe_gradient,
        ...
        print("Cumulative Rothe error: %f. Estimated (cum): %f, Estimated (init): %f"
              %(self.accumulated_error,self.accumulated_error/(self.t+self.h)*100,np.sqrt(rotheerror)/self.h*100))
        ...
        energy=abs(np.conj(best_lin_params).T@new_WF.calculate_Hamiltonian()@np.array(best_lin_params))
        ...
        if sqrt(mask_err)>1e-3:
            ...
            hess_inv0=np.diag(1/abs(grad0+1e-15*np.array(len(grad0))))              
            start=time.time()                                                   
            solver=minimize(self.rothe_error_mask_update,best_nonlin_params,method="BFGS",jac=self.rothe_gradient_mask,
        ...
        refit=False
        if refit and divisible_by_10_timestep and not added and self.t>0.5 and self.ng_initial>1:
            least_important_gaussian=np.argmin(abs(self.params_old_lin)) #A heuristic to pic the least important Gaussian
            print("Least important Gaussian is %d with absolute linear coefficient %f"%(least_important_gaussian,abs(self.params_old_lin[least_important_gaussian])))
            ...
            hess_inv0=np.diag(1/abs(grad0+1e-8*np.array(len(grad0))))  
            solver=minimize(self.overlap_refit_error,new_nonlin,method="BFGS",jac=self.overlap_refit_gradient,
                            options={"maxiter":2000,"gtol":1e-16, "hess_inv0":hess_inv0})
            ...
            if sqrt(new_ore)<1e-3 and False:
                ...
                full_nonlin=np.concatenate((np.array(self.params_old_nonlin).flatten(),newest_nonlin))
                full_nonlin=np.reshape(full_nonlin,(-1,n*(n+3)))
                full_lin=np.concatenate((self.params_old_lin,np.zeros(len(self.params_old_lin)-1)))
        ...
        while len(left)>0:
            ...
            new_nonlin_params=np.array(new_nonlin_params); new_lin_params=np.array(new_lin_params)
    ...
    def time_evolver(self):
        while self.t<=self.T:
            ...
            with h5py.File(self.filename, "r+") as data_file:
                try:
                    data_file.create_dataset("parameters_t=%.3f"%self.t,data=np.reshape(self.params_old_nonlin,(-1,n*(n+3))))
