...
class WF:
    def __init__(self,nonlin_params,lin_params,lambda_=0.1,normalization=1,calculate_Gradient=True,h=1e-3,onlyX1X2=False):
        ...
        self.nd=self.num_dimensions=np.rint(0.5*(-3+sqrt(4*len(nonlin_params[0])+9))).astype(int) #This is the invert formula of nparams = n*(n+3) for the number of dimensions
        ...
        if self.h>=0: #Only negative h will lead to this not being calculated
            #start=time.time()
            if self.onlyX1X2:
                x1_temp,x2_temp=calculate_all_expectation_values_full_allij(mymu_of_interest,mymatrices_of_interest,myovlp_of_interest,order=2)
                ...
                x1_temp,x2_temp,x3_temp,x4_temp,x5_temp,x6_temp=calculate_all_expectation_values_full_allij(mymu_of_interest,mymatrices_of_interest,myovlp_of_interest)
                self.x1_temp=np.empty((myTotnumOfElements,self.nd),dtype=np.complex128)
                self.x1_temp=x1_temp
                #self.x1_temp[mymatrices_removeIndices]=0
                self.x2_temp=np.empty((myTotnumOfElements,self.nd,self.nd),dtype=np.complex128)
            ...
            if self.calculate_Gradient and not self.onlyX1X2:                
                #start=time.time()
                x8_c1,x8_c2,x8_c3,x7_c1,x7_c2,x7_c1v,x7_c2v,x7_c3v,x7_c3,x7_c4=get_x7_x8_contraction_allij(mymu,mymatrices,myovlp)
    ...
    def update_overlap_and_overlap_derivs(self,indices_new,new_nonlin_params,new_lin_params=None):
        ...
        update_sum_matrices(self.matrices, indices_to_update, self.sum_matrices, self.sum_matrices_inv) #This updates ALL sum matrices, not just the ones of this rank
        ...
        try :
            ...
            self.updatemyderivativeintermediates(indices_new[0],new_ij_indices) #Definitely the most time consuming step!!
            self.intermediateshavebeenupdated=True
        self.new_x1_temp,self.new_x2_temp=calculate_all_expectation_values_full_allij(mymu_list,mycross_mat_inv_list,myovlp_list,order=2)
    ...
    def setupintermediates(self):
        ...
        self.IabLj=np.zeros((ng,nd,nd,nd,nd),dtype=np.complex128)#Here, self.IabLj[j,a,b,:,:] #Gives me the IabLj matrix for a given j, a, b
        ...
        self.sum_matrices,self.sum_matrices_inv=make_sum_matrices(self.matrices,np.zeros((ng,ng,nd,nd),dtype=np.complex128),None) #Those matrices are changed in-place
    ...
    def updateintermediates(self,ind_min,ind_max,new_nonlin_params):
        ...
        for i in range(ng_new):
            ...
            self.K_matrices[ind_min+i,:,:][mytril_indices]=new_nonlin_params[i][int(0.5*nd*(nd+1)):(nd*(nd+1))]
        self.vectors[ind_min:ind_max+1]=new_nonlin_params[:,(nd*(nd+1)):(nd*(nd+1))+nd]+1j*new_nonlin_params[:,nd*(nd+1)+nd:]
        self.matrices[ind_min:ind_max+1]=einsum("ijk,ilk->ijl",self.L_matrices[ind_min:ind_max+1],self.L_matrices[ind_min:ind_max+1])
        self.matrices[ind_min:ind_max+1]+=1j*(self.K_matrices[ind_min:ind_max+1]+einsum("ijk->ikj",self.K_matrices[ind_min:ind_max+1]))
        self.matrices_squared[ind_min:ind_max+1]=einsum("ikx,ixl->ikl",self.matrices[ind_min:ind_max+1],self.matrices[ind_min:ind_max+1])
        self.matrices_squared_conj[ind_min:ind_max+1]=conj(self.matrices_squared[ind_min:ind_max+1])
        self.vmmips[ind_min:ind_max+1]=einsum("ia,iab,ib->i",self.vectors[ind_min:ind_max+1],self.matrices_squared[ind_min:ind_max+1],self.vectors[ind_min:ind_max+1])
        self.cjs[ind_min:ind_max+1]=einsum("jii->j",self.matrices[ind_min:ind_max+1])-2*self.vmmips[ind_min:ind_max+1]
        self.cis[ind_min:ind_max+1]=np.conj(self.cjs[ind_min:ind_max+1])
        self.omega_T[ind_min:ind_max+1]=einsum("ik,ikl->il",self.vectors[ind_min:ind_max+1],self.matrices_squared[ind_min:ind_max+1])
        ...
        self.IabLj[ind_min:ind_max+1]=self.IabLj[ind_min:ind_max+1]+np.swapaxes(self.IabLj[ind_min:ind_max+1],-1,-2)
        self.Kabmuj[ind_min:ind_max+1]=einsum("jablk,jk->jabl",self.Kab[ind_min:ind_max+1],self.vectors[ind_min:ind_max+1])
        self.IabLjmuj[ind_min:ind_max+1]=einsum("jablk,jk->jabl",self.IabLj[ind_min:ind_max+1],self.vectors[ind_min:ind_max+1])
        self.mKm[ind_min:ind_max+1]=einsum("jk,jabk->jab",self.vectors[ind_min:ind_max+1],self.Kabmuj[ind_min:ind_max+1])
        self.mLm[ind_min:ind_max+1]=einsum("jk,jabk->jab",self.vectors[ind_min:ind_max+1],self.IabLjmuj[ind_min:ind_max+1])
    ...
    def calculate_overlap_matrix_diagonals(self):
        ...
        diagonal_exponents=einsum("ak,akl,al->a",bvec_calc,diagonal_sum_matrices_inv,bvec_calc)-vmips-vmips.conj()
        diagonal_sum_matrix =np.diagonal(self.sum_matrices,axis1=0,axis2=1).transpose([2,0,1]) #Same as einsum('iijk->ijk', self.sum_matrices)
    ...
    def calculate_myoverlapmatrix(self,row_indices,col_indices):
        ...
        myexponents=einsum("xa,xab,xb->x",mybvecouter,mysummatricesinv,mybvecouter)-myvmipsouter-0.5*mydiagonalexponentsouter
    ...
    def calculate_full_overlap_deriv_matrix(self,start_index_WFnew):
        ...
        if ic.size>0:
            ...
            D_tensor[ivc, indices_j_vc-ngMs, :self.ab_iter] = calculate_LK_derivs(ic, jvc, self.IabLj_list, self.IabLjmuj_list, conj=True) 
            D_tensor[ivc, indices_j_vc-ngMs, self.ab_iter:2*self.ab_iter] = 1j*calculate_LK_derivs(ic,jvc,self.Kab_list,self.Kabmuj_list,conj=True) 
            D_tensor[ivc, indices_j_vc-ngMs, nd*(nd+1):nd*(nd+2)] = calculate_mu_derivs(ic,jvc,conj=True) 

        if inc.size>0:
            D_tensor[ivnc, indices_j_vnc-ngMs, :self.ab_iter] = calculate_LK_derivs(inc,jvnc,self.IabLj_list,self.IabLjmuj_list,conj=False) 
            D_tensor[ivnc, indices_j_vnc-ngMs, self.ab_iter:2*self.ab_iter] = 1j*calculate_LK_derivs(inc,jvnc,self.Kab_list,self.Kabmuj_list,conj=False)
            D_tensor[ivnc, indices_j_vnc-ngMs, nd*(nd+1):nd*(nd+2)] = calculate_mu_derivs(inc,jvnc,conj=False)
    ...
    def calculate_full_potential_deriv_matrix(self,start_index_WFnew):
        ...
        def calculate_LK_derivs(indices,j_values,i_values,mat,matmuj,contr3,contr2):
            return einsum("xnm,cxnm->xc",contr2, mat[:, j_values, :, :])  + einsum("xn,cxn->xc",contr3, matmuj[:, j_values])
        ...
        if ic.size>0:
            ...
            D_tensor_alt[ivc, indices_j_vc, :self.ab_iter] = calculate_LK_derivs(ic,jvc,ivc, self.IabLj_list, self.IabLjmuj_list,C1,C2) 
            D_tensor_alt[ivc, indices_j_vc, self.ab_iter:2*self.ab_iter] = 1j*calculate_LK_derivs(ic,jvc,ivc,self.Kab_list,self.Kabmuj_list,C1,C2) 
            D_tensor_alt[ivc, indices_j_vc, nd*(nd+1):nd*(nd+2)] = calculate_mu_derivs(ic,jvc,ivc,conj=conj) 

        if inc.size>0:
            conj=False
            C1,C2=calculate_temp_mats(inc,jvnc,ivnc,conj=conj)
            D_tensor_alt[ivnc, indices_j_vnc, :self.ab_iter] = calculate_LK_derivs(inc,jvnc,ivnc,self.IabLj_list,self.IabLjmuj_list,C1,C2) 
            D_tensor_alt[ivnc, indices_j_vnc, self.ab_iter:2*self.ab_iter] = 1j*calculate_LK_derivs(inc,jvnc,ivnc,self.Kab_list,self.Kabmuj_list,C1,C2)
            D_tensor_alt[ivnc, indices_j_vnc, nd*(nd+1):nd*(nd+2)] = calculate_mu_derivs(inc,jvnc,ivnc,conj=conj) 
    ...
    def calculate_full_kinetic_energy_deriv(self,start_index_WFnew): 
        ...
        if ic.size>0:
            ...
            D_tensor[ivc, indices_j_vc, :self.ab_iter] = calculate_LK_derivs(ic,jvc,ivc, self.IabLj_list, self.IabLjmuj_list,C1,C2) 
            D_tensor[ivc, indices_j_vc, self.ab_iter:2*self.ab_iter] = 1j*calculate_LK_derivs(ic,jvc,ivc,self.Kab_list,self.Kabmuj_list,C1,C2) 
        ...
        if inc.size>0:
            ...
            D_tensor[ivnc, indices_j_vnc, :self.ab_iter] = calculate_LK_derivs(inc,jvnc,ivnc,self.IabLj_list,self.IabLjmuj_list,C1,C2) 
            D_tensor[ivnc, indices_j_vnc, self.ab_iter:2*self.ab_iter] = 1j*calculate_LK_derivs(inc,jvnc,ivnc,self.Kab_list,self.Kabmuj_list,C1,C2)
            D_tensor[ivnc, indices_j_vnc, nd*(nd+1):nd*(nd+2)] = calculate_mu_derivs(inc,jvnc,ivnc,conj=conj) 
        D_tensor[:,:,nd*(nd+2):nd*(nd+3)]=1j*D_tensor[:,:,nd*(nd+1):nd*(nd+2)]
        return D_tensor
    ...
    def calculate_kinetic_energy_squared(self):
        ...
        kinetic_squared_temp=-2*einsum("x,xkl,xkl->x",self.ci_of_interest,self.matrices_j_squared_of_interest,self.x2_temp)
        kinetic_squared_temp+=-2*einsum("x,xkl,xkl->x",self.cj_of_interest,self.matrices_i_squared_of_interest,self.x2_temp)
        kinetic_squared_temp+=16*einsum("xk,xl,xkl->x",self.rho_T_conj_of_interest,self.omega_T_of_interest,self.x2_temp) 
        ...
        kinetic_squared_temp+=-8*einsum("xk,xlm,xklm->x",self.omega_T_of_interest,self.matrices_i_squared_of_interest,self.x3_temp)
        kinetic_squared_temp+=-8*einsum("xk,xlm,xklm->x",self.rho_T_conj_of_interest,self.matrices_j_squared_of_interest,self.x3_temp)

        kinetic_squared_temp+=einsum("x,x,x->x",self.ci_of_interest,self.cj_of_interest,self.myovlp)
        kinetic_squared_temp+=4*einsum("xkl,xmn,xklmn->x",self.matrices_i_squared_of_interest,self.matrices_j_squared_of_interest,self.x4_temp)
    ...
    def calculate_full_potential_squared_deriv_matrix(self,start_index_WFnew):
        ...
        def calculate_temp_mats(indices,j_values,i_values,conj=True):
            ...
            super_contr1+=2/9*self.lambda_sq*x7_c1v+2*self.lambda_sq*x7_c2v+-4/3*self.lambda_sq*x7_c3v
    ...
    def calculate_full_kinetic_energy_squared_deriv(self,start_index_WFnew):
        ...
        def calculate_LK_derivs(indices,j_values,i_values,Asquaredderiv,cj_deriv,omega_deriv,mat,matmuj,const,temps):
        ...
        if ic.size>0:
            ...
            D_tensor_alt[ivc, indices_j_vc, :self.ab_iter] = calculate_LK_derivs(ic,jvc,ivc,Asquared_derivs_I_list,cj_derivs_I_list,omega_derivs_I_list, self.IabLj_list, self.IabLjmuj_list,mLm_list,temps) 
            D_tensor_alt[ivc, indices_j_vc, self.ab_iter:2*self.ab_iter] = 1j*calculate_LK_derivs(ic,jvc,ivc,Asquared_derivs_K_list,cj_derivs_K_list,omega_derivs_K_list, self.Kab_list, self.Kabmuj_list,mKm_list,temps) 
            D_tensor_alt[ivc, indices_j_vc, nd*(nd+1):nd*(nd+2)] = calculate_mu_derivs(ic,jvc,ivc,temps[6:]) 

        if inc.size>0:
            conj=False
            temps=calculate_temp_mats(inc,jvnc,ivnc,conj=conj)
            D_tensor_alt[ivnc, indices_j_vnc, :self.ab_iter] = calculate_LK_derivs(inc,jvnc,ivnc,Asquared_derivs_I_list,cj_derivs_I_list,omega_derivs_I_list, self.IabLj_list, self.IabLjmuj_list,mLm_list,temps) 
            D_tensor_alt[ivnc, indices_j_vnc, self.ab_iter:2*self.ab_iter] = 1j*calculate_LK_derivs(inc,jvnc,ivnc,Asquared_derivs_K_list,cj_derivs_K_list,omega_derivs_K_list, self.Kab_list, self.Kabmuj_list,mKm_list,temps) 
            D_tensor_alt[ivnc, indices_j_vnc, nd*(nd+1):nd*(nd+2)] = calculate_mu_derivs(inc,jvnc,ivnc,temps[6:]) 
        D_tensor_alt[:,:,nd*(nd+2):nd*(nd+3)]=1j*D_tensor_alt[:,:,nd*(nd+1):nd*(nd+2)]
        return D_tensor_alt
    def calculate_potential_times_kinetic_energy(self):
        self.potential_times_kinetic=np.zeros_like(self.overlap)
        potential_temp=2/3*self.lambda_*einsum("xmmmkl,xkl->x",self.x5_temp[:,1:,1:,1:,:,:],self.matrices_squared_of_interest)
        potential_temp+=-2*self.lambda_*einsum("xmmmkl,xkl->x",self.x5_temp[:,:-1,:-1,1:,:,:],self.matrices_squared_of_interest)
        potential_temp+=-4/3*self.lambda_*einsum("xklll,xk->x",self.x4_temp[:,:,1:,1:,1:],self.omega_T_of_interest)
        potential_temp+=4*self.lambda_*einsum("xklll,xk->x",self.x4_temp[:,:,:-1,:-1,1:],self.omega_T_of_interest)
        ...
        potential_temp=2/3*self.lambda_*einsum("xmmmkl,xkl->x",self.x5_temp[:,1:,1:,1:,:,:],self.matrices_i_squared_of_interest)
        potential_temp+=-2*self.lambda_*einsum("xmmmkl,xkl->x",self.x5_temp[:,:-1,:-1,1:,:,:],self.matrices_i_squared_of_interest)
        potential_temp+=-4/3*self.lambda_*einsum("xklll,xk->x",self.x4_temp[:,:,1:,1:,1:],self.rho_T_conj_of_interest)
        potential_temp+=4*self.lambda_*einsum("xklll,xk->x",self.x4_temp[:,:,:-1,:-1,1:],self.rho_T_conj_of_interest)
    ...
    def calculate_full_potential_times_kinetic_deriv(self,start_index_WFnew):
        ...
        def calculate_temp_mats(indices,j_values,i_values,conj=True):
            ...
            temp1+=einsum("x,x->x",mycis,-self.lambda_*x3e_c1_mat+1/3*self.lambda_*x3e_c2_mat- 0.5*x2e_c_mat)
            temp1+=-einsum("x->x", 4*self.lambda_*x4e_c1_mat)
            temp1+=einsum("x,x->x",mycjs,+self.lambda_*1/3* x3e_c2_mat- 0.5*x2e_c_mat-self.lambda_*x3e_c1_mat)
    ...
    def calculate_Hamiltonian_deriv(self,start_index_WFnew,include_kinetic=True):
        ...
        if self.rank==0:
            if include_kinetic:
                H_deriv_full+=-0.5*einsum("ij,jk->ijk",self.potential[:,si:]+self.kinetic[:,si:],self.fodmd[si:,:])
            else:
    ...
    def calculate_Hamiltonian_squared_deriv(self,start_index_WFnew,include_kinetic=True):
        ...
        if self.rank==0:
            if include_kinetic:
                H2_deriv_full+=-0.5*einsum("ij,jk->ijk",self.potential_squared[:,si:]+self.kinetic_energy_squared[:,si:]+self.potential_times_kinetic[:,si:],self.fodmd[si:,:])
            else:
