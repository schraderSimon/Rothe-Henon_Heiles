...
for i, partition in enumerate(fourth_partitions):
    fourth_partition_strings.append(
        "a%s%s%s%s" % (partition[0][0], partition[0][1], partition[1][0], partition[1][1])
    )
...
six_partition_string_transpose = [
    [0, 1, 3, 4, 5, 2, 6],
    [0, 1, 3, 5, 4, 6, 2],
    [0, 1, 3, 5, 2, 6, 4],
    [0, 1, 3, 5, 2, 4, 6],
...
def make_integrals(matrix, order=2):
    ...
    if order == 4:
        fourth_order_sum_elements = (
            input_matrix[:, :, :, np.newaxis, np.newaxis]
            * input_matrix[:, np.newaxis, np.newaxis, :, :]
        )
        fourth_order_integrals = summaker(fourth_order_sum_elements, fourth_partition_strings)
        return fourth_order_integrals
    sixth_order_sum_elements = (
        input_matrix[:, :, :, np.newaxis, np.newaxis, np.newaxis, np.newaxis]
        * input_matrix[:, np.newaxis, np.newaxis, :, :, np.newaxis, np.newaxis]
        * input_matrix[:, np.newaxis, np.newaxis, np.newaxis, np.newaxis, :, :]
    )
    if order == 6:
        ...
        for i in range(len(six_partition_string_transpose)):
            sixth_order_integrals += sixth_order_sum_elements.transpose(
                six_partition_string_transpose[i]
            )
        return sixth_order_integrals
    eight_order_sum_elements = einsum(
        "aijklmn,aop->aijklmnop", sixth_order_sum_elements, input_matrix
    )
    if order == 8:
...
def find_returnval6_reduced(mu4, mu2, inp, fourth_expecs, reduced_indices6, ti64, ti62):
    i, j, k, l, m, n = np.array(
        reduced_indices6
    ).T  # extract i, j, k, l, m, and n from indices
...
def makeFulltensor5(tensor_reduced, all_permutations_list, dim=4):
    ...
    full_tensors = np.zeros(
        (tensor_reduced.shape[0], dim, dim, dim, dim, dim), dtype=tensor_reduced.dtype
    )
    for index, all_permutations in enumerate(all_permutations_list):
        ap = np.array(all_permutations)
        full_tensors[(np.s_[:], ap[:, 0], ap[:, 1], ap[:, 2], ap[:, 3], ap[:, 4])] = (
            tensor_reduced[:, index]
        )
    return full_tensors
...
def makeFulltensor6(tensor_reduced, all_permutations_list, dim=4):
    tensor_reduced = tensor_reduced.reshape(tensor_reduced.shape[0], -1, 1)
    full_tensors = np.zeros(
        (tensor_reduced.shape[0], dim, dim, dim, dim, dim, dim), dtype=tensor_reduced.dtype
    )
    for index, all_permutations in enumerate(all_permutations_list):
        ap = np.array(all_permutations)
        full_tensors[
            (np.s_[:], ap[:, 0], ap[:, 1], ap[:, 2], ap[:, 3], ap[:, 4], ap[:, 5])
        ] = tensor_reduced[:, index]
    return full_tensors
...
def calculate_all_expectation_values_full_allij(mu, matrix, ovlps, order=6):
    ...
    returnvals.append(
        mu3_norm
        + einsum("n,ni,njk->nijk", ovlps, mu, inp)
        + einsum("n,nj,nik->nijk", ovlps, mu, inp)
        + einsum("n,nk,nij->nijk", ovlps, mu, inp)
    )
    ...
    if dimension > 6:
        ...
        mu5 = (
            mu[:, :, np.newaxis, np.newaxis, np.newaxis, np.newaxis] * mu4[:, np.newaxis, ...]
        )
        ...
        ind = dimension - 2
        returnval5_temp = find_returnval5_reduced(
            mu,
            mu3,
            inp,
            fourth_expecs,
            reduced_indices5_list[ind],
            transposed_indices_53_lists[ind],
            transposed_indices_51_lists[ind],
        )
        returnval5 = makeFulltensor5(
            returnval5_temp, all_permutations_new_lists5[ind], dim=dimension
        )
    ...
    if dimension > 6:
        ...
        returnval6 += (
            mu[:, :, np.newaxis, np.newaxis, np.newaxis, np.newaxis, np.newaxis]
            * mu5[:, np.newaxis, ...]
        )
        temp1 = (
            mu4[..., np.newaxis, np.newaxis]
            * inp[:, np.newaxis, np.newaxis, np.newaxis, np.newaxis, :, :]
        )
        temp2 = (
            mu2[..., np.newaxis, np.newaxis, np.newaxis, np.newaxis]
            * fourth_expecs[:, np.newaxis, np.newaxis, ...]
        )
        returnval6 += temp1
        returnval6 += temp2
        ...
        returnval6_temp = find_returnval6_reduced(
            mu4,
            mu2,
            inp,
            fourth_expecs,
            reduced_indices6_list[ind],
            transposed_indices_64_lists[dimension - 2],
            transposed_indices_62_lists[ind],
        )

        returnval6 += makeFulltensor6(
            returnval6_temp, all_permutations_new_lists6[ind], dim=dimension
        )
...
def get_x7_x8_contraction_allij(mu, matrix, ovlps):
    ...
    contraction_1 += 9 * einsum(
        "ai,aj,ak,ak,al,al,akl->aij", mu, mu, mu1, mu1, mu1, mu1, inp[:, 1:, 1:]
    )
    ...
    contraction_1 += 3 * einsum(
        "ai,aj,ak,ak,ak->aij", mu, mu, mu1, mu1, fourth_111_temp[:, 1:]
    )
    ...
    temp = 6 * einsum(
        "ai,ak,ak,al,ajkll->aij", mu, mu1, mu1, mu1, fourth_expecs[:, :, 1:, 1:, 1:]
    )
    ...
    contraction_1 += temp + np.transpose(temp, [0, 2, 1])
    contraction_1 += 9 * einsum(
        "ak,ak,al,al,aijkl->aij", mu1, mu1, mu1, mu1, fourth_expecs[:, :, :, 1:, 1:]
    )
    contraction_1 += einsum("ak,ak,aj,ak,ai->aij", mu1, mu1, mu, mu1, fourth_111_temp)
    temp = 3 * einsum(
        "ak,ak,ai,al,ajkll->aij", mu1, mu1, mu, mu1, fourth_expecs[:, :, 1:, 1:, 1:]
    )
    contraction_1 += temp + np.transpose(temp, [0, 2, 1])
    ...
    contraction_1 += temp + np.transpose(temp, [0, 2, 1])
    contraction_1 += 9 * einsum(
        "ak,al,aijkkll->aij", mu1, mu1, sixth_expecs[:, :, :, 1:, 1:, 1:, 1:]
    )
    akklall_contr = einsum("akl,all->ak", inp[:, 1:, 1:], inp[:, 1:, 1:])
    contraction_1 += 9 * einsum("aij,akk,ak->aij", inp, inp[:, 1:, 1:], akklall_contr)
    contraction_1 += 36 * einsum(
        "aik,ajk,ak->aij", inp[:, :, 1:], inp[:, :, 1:], akklall_contr
    )
    contraction_1 += 18 * einsum(
        "aik,ajl,akk,all->aij", inp[:, :, 1:], inp[:, :, 1:], inp[:, 1:, 1:], inp[:, 1:, 1:]
    )
    contraction_1 += 36 * einsum(
        "aik,ajl,akl,akl->aij", inp[:, :, 1:], inp[:, :, 1:], inp[:, 1:, 1:], inp[:, 1:, 1:]
    )
    contraction_1 += 6 * einsum(
        "aij,akl,akl,akl->aij", inp, inp[:, 1:, 1:], inp[:, 1:, 1:], inp[:, 1:, 1:]
    )
    ...
    t_contraction_2 += 4 * einsum(
        "ak,al,akkll->a", mum1, mum1, fourth_expecs[:, :-1, 1:, :-1, 1:]
    )
    t_contraction_2 += 2 * einsum(
        "ak,al,akkll->a", mum1, mu1, fourth_expecs[:, :-1, 1:, :-1, :-1]
    )
    t_contraction_2 += 2 * einsum(
        "ak,al,akkll->a", mu1, mum1, fourth_expecs[:, :-1, :-1, :-1, 1:]
    )
    ...
    temp = einsum(
        "ai,ak,ak,al,ajkll->aij", mu, mum1, mu1, mu1, fourth_expecs[:, :, :-1, :-1, :-1]
    )
    contraction_2 += temp + np.transpose(temp, [0, 2, 1])
    temp = 4 * einsum(
        "ai,ak,al,al,ajkkl->aij", mu, mum1, mum1, mu1, fourth_expecs[:, :, :-1, 1:, :-1]
    )
    contraction_2 += temp + np.transpose(temp, [0, 2, 1])
    contraction_2 += 2 * einsum("a,aij->aij", mu_m_contr_m1m11, mu1_fem1m1contr)
    contraction_2 += 2 * einsum(
        "ak,ak,al,al,aijkl->aij", mum1, mum1, mum1, mu1, fourth_expecs[:, :, :, 1:, :-1]
    )
    contraction_2 += 4 * einsum(
        "ak,ak,al,al,aijkl->aij", mum1, mu1, mum1, mu1, fourth_expecs[:, :, :, :-1, :-1]
    )
    contraction_2 += 2 * einsum("a,aij->aij", mu_m_contr_m1m11, mum1_fem11contr)
    temp = einsum(
        "ai,ak,al,al,ajkkl->aij", mu, mu1, mum1, mu1, fourth_expecs[:, :, :-1, :-1, :-1]
    )
    contraction_2 += temp + 2 * np.transpose(temp, [0, 2, 1])
    contraction_2 += 2 * einsum("a,aij->aij", mu_m_contr_m1m11, mum1_fem11contr)
    contraction_2 += 2 * einsum(
        "ak,ak,al,al,aijkl->aij", mum1, mu1, mum1, mum1, fourth_expecs[:, :, :, :-1, 1:]
    )
    contraction_2 += einsum("ak,ak,aj,ak,ai->aij", mum1, mu1, mu, mum1, fourth_m1m11_temp)
    temp = 2 * einsum(
        "ak,ak,ai,al,ajkll->aij", mum1, mu1, mu, mum1, fourth_expecs[:, :, :-1, :-1, 1:]
    )
    contraction_2 += temp + np.transpose(temp, [0, 2, 1])
    temp = einsum(
        "ak,ak,ai,al,ajkll->aij", mum1, mu1, mu, mu1, fourth_expecs[:, :, :-1, :-1, :-1]
    )
    contraction_2 += temp + np.transpose(temp, [0, 2, 1])
    temp = 2 * einsum(
        "ai,ak,al,al,ajkkl->aij", mu, mum1, mum1, mum1, fourth_expecs[:, :, :-1, 1:, 1:]
    )
    contraction_2 += temp + np.transpose(temp, [0, 2, 1])
    contraction_2 += einsum(
        "ak,aj,al,al,aikkl->aij", mu1, mu, mum1, mum1, fourth_expecs[:, :, :-1, :-1, 1:]
    )
    contraction_2 += einsum(
        "ak,ak,al,al,aijkl->aij", mum1, mum1, mum1, mum1, fourth_expecs[:, :, :, 1:, 1:]
    )
    temp = einsum(
        "ai,ak,ak,al,ajkll->aij", mu, mum1, mum1, mu1, fourth_expecs[:, :, 1:, :-1, :-1]
    )
    contraction_2 += temp + np.transpose(temp, [0, 2, 1])
    temp = 2 * einsum(
        "ai,ak,ak,al,ajkll->aij", mu, mum1, mu1, mum1, fourth_expecs[:, :, :-1, :-1, 1:]
    )
    contraction_2 += temp + np.transpose(temp, [0, 2, 1])

    contraction_2 += einsum(
        "ai,ak,al,al,ajkkl->aij", mu, mu1, mum1, mum1, fourth_expecs[:, :, :-1, :-1, 1:]
    )
    contraction_2 += einsum(
        "ak,aj,al,al,aikkl->aij", mu1, mu, mum1, mu1, fourth_expecs[:, :, :-1, :-1, :-1]
    )
    ...
    contraction_2 += 4 * einsum(
        "ak,al,aijkkll->aij", mum1, mum1, sixth_expecs[:, :, :, :-1, 1:, :-1, 1:]
    )
    contraction_2 += 4 * einsum(
        "ak,al,aijkkll->aij", mum1, mu1, sixth_expecs[:, :, :, :-1, 1:, :-1, :-1]
    )
    contraction_2 += einsum(
        "ak,al,aijkkll->aij", mu1, mu1, sixth_expecs[:, :, :, :-1, :-1, :-1, :-1]
    )
    ...
    contraction_2 += 2 * einsum(
        "al,al,aijl->aij", mum1, mu1, sixth_m1m11kkk_contr[:, :, :, :-1]
    )
    contraction_2 += 2 * einsum(
        "aik,ajk,akl,all->aij",
        inp[:, :, :-1],
    ...
    contraction_2 += 8 * einsum(
        "ail,ajl,al->aij", inp[:, :, :-1], inp[:, :, 1:], aklakk_contr2
    )

    contraction_2 += 8 * einsum(
        "ail,ajl,al->aij", inp[:, :, 1:], inp[:, :, :-1], aklakk_contr2
    )
    contraction_2 += 16 * einsum(
        "ail,ajk,akl,akl->aij",
        inp[:, :, 1:],
        inp[:, :, :-1],
        inp[:, :-1, :-1],
    ...
    t_contraction_3 += 6 * einsum(
        "ak,al,akkll->a", mu1, mum1, fourth_expecs[:, 1:, 1:, :-1, 1:]
    )
    t_contraction_3 += 3 * einsum(
        "ak,al,akkll->a", mu1, mu1, fourth_expecs[:, 1:, 1:, :-1, :-1]
    )
    ...
    contraction_3 += 12 * einsum(
        "ai,ak,al,al,ajkkl->aij", mu, mum1, mu1, mu1, fourth_expecs[:, :, 1:, :-1, 1:]
    )
    contraction_3 += 6 * einsum(
        "ai,ak,al,al,ajkkl->aij", mu, mu1, mu1, mu1, fourth_expecs[:, :, :-1, :-1, 1:]
    )
    ...
    contraction_3 += 6 * einsum(
        "ak,ak,al,al,aijkl->aij", mu1, mu1, mum1, mu1, fourth_expecs[:, :, :, 1:, :-1]
    )
    contraction_3 += einsum("ai,a,aj->aij", mu, mu_m_contr_m1m11, fourth_111_temp)
    ...
    contraction_3 += 3 * einsum(
        "ak,ak,al,al,aijkl->aij", mu1, mu1, mum1, mum1, fourth_expecs[:, :, :, 1:, 1:]
    )
    ...
    contraction_3 += 6 * einsum(
        "ak,al,aijkkll->aij", mu1, mum1, sixth_expecs[:, :, :, 1:, 1:, :-1, 1:]
    )
    contraction_3 += 3 * einsum(
        "ak,al,aijkkll->aij", mu1, mu1, sixth_expecs[:, :, :, 1:, 1:, :-1, :-1]
    )
    ...
    contraction_3 += 6 * einsum(
        "ajl,aik,akl,akl->aij", inp[:, :, 1:], inp[:, :, :-1], inp[:, :-1, 1:], inp[:, 1:, 1:]
    )
    contraction_3 += 6 * einsum(
        "aik,ajl,akl,akl->aij", inp[:, :, 1:], inp[:, :, :-1], inp[:, 1:, :-1], inp[:, 1:, 1:]
    )
    ...
    contraction_3 += temp + np.transpose(temp, [0, 2, 1])
    contraction_3 += 12 * einsum(
        "aik,ajk,all,akl->aij", inp[:, :, 1:], inp[:, :, :-1], inp[:, 1:, 1:], inp[:, :-1, 1:]
    )

    contraction_3 += 6 * einsum(
        "aik,ajk,akl,all->aij", inp[:, :, 1:], inp[:, :, 1:], inp[:, 1:, 1:], inp[:, :-1, :-1]
    )
    contraction_3 += 6 * einsum(
        "aik,ajl,akk,all->aij", inp[:, :, 1:], inp[:, :, :-1], inp[:, 1:, 1:], inp[:, :-1, 1:]
    )
    ...
    contr_temp += 6 * einsum(
        "akk,akl,all->a", inp[:, 1:, 1:], inp[:, 1:, :-1], inp[:, :-1, 1:]
    )
    contraction_3 += einsum("aij,a->aij", inp, contr_temp)

    x7_e_contracted_first = einsum("ai,aj,ak,ak,a->aij", mu, mu, mu, mu, mu_m_contr_m1m11)
    x7_e_contracted_first += 2 * einsum(
        "ai,aj,ak,ak,a->aij", mu, mu, mu, mu, mum1_in_m11_contr
    )
    x7_e_contracted_first += 1 * einsum(
        "ai,aj,ak,ak,al,all->aij", mu, mu, mu, mu, mu1, inp[:, :-1, :-1]
    )
    x7_e_contracted_first += 2 * einsum(
        "ai,aj,ak,al,al,akl->aij", mu, mu, mu, mum1, mum1, inp[:, :, 1:]
    )
    x7_e_contracted_first += 4 * einsum(
        "ai,aj,ak,al,al,akl->aij", mu, mu, mu, mum1, mu1, inp[:, :, :-1]
    )
    ...
    x7_e_contracted_first += 2 * einsum(
        "ak,al,al,aijkl->aij", mu, mum1, mum1, fourth_expecs[:, :, :, :, 1:]
    )
    x7_e_contracted_first += 4 * einsum(
        "ak,al,al,aijkl->aij", mu, mum1, mu1, fourth_expecs[:, :, :, :, :-1]
    )
    ...
    x7_e_contracted_first += 2 * einsum(
        "ak,aijkk->aij", mum1, sixth_kk_kontr[:, :, :, :-1, 1:]
    )
    x7_e_contracted_first += 1 * einsum(
        "ak,aijkk->aij", mu1, sixth_kk_kontr[:, :, :, :-1, :-1]
    )
    ...
    x7_e_contracted_firstV += 9 * einsum(
        "ai,ak,ak,al,al,akl->ai", mu, mu1, mu1, mu1, mu1, inp[:, 1:, 1:]
    )
    x7_e_contracted_firstV += 6 * einsum(
        "ak,ak,a,aik->ai", mu1, mu1, mu_m_contr_111, inp[:, :, 1:]
    )
    x7_e_contracted_firstV += 6 * einsum(
        "ai,ak,a,akk->ai", mu, mu1, mu_m_contr_111, inp[:, 1:, 1:]
    )
    x7_e_contracted_firstV += 3 * einsum(
        "ai,ak,ak,ak->ai", mu, mu1, mu1, fourth_111_temp[:, 1:]
    )
    ...
    x7_e_contracted_firstV += 9 * einsum(
        "ak,ak,al,aikll->ai", mu1, mu1, mu1, fourth_expecs[:, :, 1:, 1:, 1:]
    )
    x7_e_contracted_firstV += 9 * einsum(
        "ak,al,al,aikkl->ai", mu1, mu1, mu1, fourth_expecs[:, :, 1:, 1:, 1:]
    )
    ...
    x7_e_contracted_secondV += 2 * einsum(
        "ai,a,a->ai", mu, mu_m_contr_m1m11, mum1_in_m11_contr
    )
    x7_e_contracted_secondV += 1 * einsum(
        "ai,a,al,all->ai", mu, mu_m_contr_m1m11, mu1, inp[:, :-1, :-1]
    )
    x7_e_contracted_secondV += 1 * einsum(
        "ai,ak,ak,al,al,akl->ai", mu, mum1, mum1, mum1, mum1, inp[:, 1:, 1:]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ai,ak,ak,al,al,akl->ai", mu, mum1, mum1, mum1, mu1, inp[:, 1:, :-1]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ai,ak,ak,al,al,akl->ai", mu, mum1, mu1, mum1, mum1, inp[:, :-1, 1:]
    )
    x7_e_contracted_secondV += 4 * einsum(
        "ai,ak,ak,al,al,akl->ai", mu, mum1, mu1, mum1, mu1, inp[:, :-1, :-1]
    )
    x7_e_contracted_secondV += 1 * einsum(
        "a,al,al,ail->ai", mu_m_contr_m1m11, mum1, mum1, inp[:, :, 1:]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "a,al,al,ail->ai", mu_m_contr_m1m11, mum1, mu1, inp[:, :, :-1]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ak,ak,a,aik->ai", mum1, mu1, mu_m_contr_m1m11, inp[:, :, :-1]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ai,ak,a,akk->ai", mu, mum1, mu_m_contr_m1m11, inp[:, :-1, 1:]
    )
    x7_e_contracted_secondV += 1 * einsum(
        "ai,ak,a,akk->ai", mu, mu1, mu_m_contr_m1m11, inp[:, :-1, :-1]
    )
    x7_e_contracted_secondV += 1 * einsum(
        "ak,ak,a,aik->ai", mum1, mum1, mu_m_contr_m1m11, inp[:, :, 1:]
    )
    x7_e_contracted_secondV += 1 * einsum(
        "ai,ak,ak,ak->ai", mu, mum1, mum1, fourth_m1m11_temp[:, 1:]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ai,ak,ak,ak->ai", mu, mum1, mu1, fourth_m1m11_temp[:, :-1]
    )
    x7_e_contracted_secondV += 4 * einsum(
        "ai,ak,al,akkll->ai", mu, mum1, mum1, fourth_expecs[:, :-1, 1:, :-1, 1:]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ai,ak,al,akkll->ai", mu, mum1, mu1, fourth_expecs[:, :-1, 1:, :-1, :-1]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ai,ak,al,akkll->ai", mu, mu1, mum1, fourth_expecs[:, :-1, :-1, :-1, 1:]
    )
    x7_e_contracted_secondV += 1 * einsum(
        "ai,ak,al,akkll->ai", mu, mu1, mu1, fourth_expecs[:, :-1, :-1, :-1, :-1]
    )
    x7_e_contracted_secondV += 1 * einsum(
        "ai,al,al,al->ai", mu, mum1, mum1, fourth_kkk_contr[:, 1:]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ai,al,al,al->ai", mu, mum1, mu1, fourth_kkk_contr[:, :-1]
    )
    ...
    x7_e_contracted_secondV += 1 * einsum(
        "ak,ak,al,aikll->ai", mum1, mum1, mu1, fourth_expecs[:, :, 1:, :-1, :-1]
    )
    x7_e_contracted_secondV += 4 * einsum(
        "ak,ak,al,aikll->ai", mum1, mu1, mum1, fourth_expecs[:, :, :-1, :-1, 1:]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ak,ak,al,aikll->ai", mum1, mu1, mu1, fourth_expecs[:, :, :-1, :-1, :-1]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ak,al,al,aikkl->ai", mum1, mum1, mum1, fourth_expecs[:, :, :-1, 1:, 1:]
    )
    x7_e_contracted_secondV += 4 * einsum(
        "ak,al,al,aikkl->ai", mum1, mum1, mu1, fourth_expecs[:, :, :-1, 1:, :-1]
    )
    x7_e_contracted_secondV += 1 * einsum(
        "ak,al,al,aikkl->ai", mu1, mum1, mum1, fourth_expecs[:, :, :-1, :-1, 1:]
    )
    x7_e_contracted_secondV += 2 * einsum(
        "ak,al,al,aikkl->ai", mu1, mum1, mu1, fourth_expecs[:, :, :-1, :-1, :-1]
    )
    x7_e_contracted_secondV += 1 * einsum("a,ai->ai", mu_m_contr_m1m11, fourth_m1m11_temp)
    ...
    x7_e_contracted_secondV += 2 * einsum(
        "al,aill->ai", mum1, sixth_m1m11kkk_contr[:, :, :-1, 1:]
    )
    x7_e_contracted_secondV += 1 * einsum(
        "al,aill->ai", mu1, sixth_m1m11kkk_contr[:, :, :-1, :-1]
    )
    ...
    x7_e_contracted_thirdV += 1 * einsum(
        "ai,a,al,all->ai", mu, mu_m_contr_111, mu1, inp[:, :-1, :-1]
    )
    x7_e_contracted_thirdV += 3 * einsum(
        "ai,ak,ak,al,al,akl->ai", mu, mu1, mu1, mum1, mum1, inp[:, 1:, 1:]
    )
    x7_e_contracted_thirdV += 6 * einsum(
        "ai,ak,ak,al,al,akl->ai", mu, mu1, mu1, mum1, mu1, inp[:, 1:, :-1]
    )
    x7_e_contracted_thirdV += 1 * einsum(
        "a,al,al,ail->ai", mu_m_contr_111, mum1, mum1, inp[:, :, 1:]
    )
    x7_e_contracted_thirdV += 2 * einsum(
        "a,al,al,ail->ai", mu_m_contr_111, mum1, mu1, inp[:, :, :-1]
    )
    x7_e_contracted_thirdV += 3 * einsum(
        "ak,ak,a,aik->ai", mu1, mu1, mu_m_contr_m1m11, inp[:, :, 1:]
    )
    x7_e_contracted_thirdV += 3 * einsum(
        "ai,ak,a,akk->ai", mu, mu1, mu_m_contr_m1m11, inp[:, 1:, 1:]
    )
    x7_e_contracted_thirdV += 3 * einsum(
        "ai,ak,ak,ak->ai", mu, mu1, mu1, fourth_m1m11_temp[:, 1:]
    )
    x7_e_contracted_thirdV += 6 * einsum(
        "ai,ak,al,akkll->ai", mu, mu1, mum1, fourth_expecs[:, 1:, 1:, :-1, 1:]
    )
    ...
    x7_e_contracted_thirdV += 1 * einsum(
        "ai,al,al,al->ai", mu, mum1, mum1, fe_lll_contr[:, 1:]
    )
    x7_e_contracted_thirdV += 2 * einsum(
        "ai,al,al,al->ai", mu, mum1, mu1, fe_lll_contr[:, :-1]
    )
    ...
    x7_e_contracted_thirdV += 2 * einsum(
        "al,aill->ai", mum1, sixth_111kkk_contr[:, :, :-1, 1:]
    )
    x7_e_contracted_thirdV += 1 * einsum(
        "al,aill->ai", mu1, sixth_111kkk_contr[:, :, :-1, :-1]
    )
    ...
    t_x7_e_contracted_fourth += 1 * einsum(
        "aj,ak,al,am,amm->ajkl", mu, mu, mu, mu1, inp[:, :-1, :-1]
    )
    t_x7_e_contracted_fourth += 2 * einsum(
        "aj,ak,am,am,alm->ajkl", mu, mu, mum1, mum1, inp[:, :, 1:]
    )
    t_x7_e_contracted_fourth += 4 * einsum(
        "aj,ak,am,am,alm->ajkl", mu, mu, mum1, mu1, inp[:, :, :-1]
    )
    t_x7_e_contracted_fourth += 2 * einsum(
        "ak,al,am,am,ajm->ajkl", mu, mu, mum1, mum1, inp[:, :, 1:]
    )
    t_x7_e_contracted_fourth += 4 * einsum(
        "ak,al,am,am,ajm->ajkl", mu, mu, mum1, mu1, inp[:, :, :-1]
    )
    ...
    t_x7_e_contracted_fourth += einsum(
        "aj,akl->ajkl", mu, mum1_fem11contr * 2 + mu1_fem1m1contr
    )
    ...
    x7_e_contracted_fourth += einsum(
        "ak,al,aij->aijkl", mu, mu, 2 * mum1_fem11contr + mu1_fem1m1contr
    )
    x7_e_contracted_fourth += 2 * einsum(
        "al,aijk->aijkl", mu, mum1mum1fe1_temp + 2 * mum1mu1fem1_temp
    )
    ...
    x7_e_contracted_fourth += 2 * einsum(
        "am,aijklmm->aijkl", mum1, sixth_expecs[:, :, :, :, :, :-1, 1:]
    )
    x7_e_contracted_fourth += 1 * einsum(
        "am,aijklmm->aijkl", mu1, sixth_expecs[:, :, :, :, :, :-1, :-1]
    )
    ...
    x7_e_contracted_fifth += einsum(
        "ak,aijl->aijkl", mu, 6 * mu1mu1fe1_temp + 2 * sixth_111_temp
    )
    x7_e_contracted_fifth += 1 * einsum("a,aijkl->aijkl", mu_m_contr_111, fourth_expecs)
    x7_e_contracted_fifth += 3 * einsum(
        "am,aijklmm->aijkl", mu1, sixth_expecs[:, :, :, :, :, 1:, 1:]
    )
...
if __name__ == "__main__":
    ...
    x8_c1, x8_c2, x8_c3, x7_c1, x7_c2, x7_c1v, x7_c2v, x7_c3v, x7_c3, x7_c4 = (
        get_x7_x8_contraction_allij(muvals, matrices, ovlps)
    )
    from calculate_efficient_tensors import *
